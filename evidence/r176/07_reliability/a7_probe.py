...
def rec(label, code, text, limit=500):
    OUT.append(f"### {label}\n{text[:limit]} [{code}]\n")
...
c1, t1 = http("POST", "/v1/execute", U, {"ask": "idem", "execution_policy": {"async": True}}, h)
rec("R-01 async execute with Idempotency-Key #1", c1, t1)
c2, t2 = http("POST", "/v1/execute", U, {"ask": "idem", "execution_policy": {"async": True}}, h)
rec("R-02 same key #2 (expect same execution_id or 200/202 replay, never a second job)", c2, t2)
try:
...
c3, t3 = http(
    "POST", "/v1/execute", U, {"ask": "DIFFERENT body", "execution_policy": {"async": True}}, h
)
rec("R-03 same key, different body => expect 409/422 conflict", c3, t3)
time.sleep(1)
c, t = http("GET", "/v1/usage", U)
rec("R-04 usage after idempotent pair (+1 conflict): expect 1 or 2 units, not 3", c, t)
# sync path with key
c4, t4 = http("POST", "/v1/execute", U, {"ask": "idem-sync"}, {"Idempotency-Key": "r176-idem-2"})
rec("R-05 sync execute with key #1", c4, t4, 200)
c5, t5 = http("POST", "/v1/execute", U, {"ask": "idem-sync"}, {"Idempotency-Key": "r176-idem-2"})
rec("R-06 sync replay #2", c5, t5, 200)
try:
    OUT.append(
        f"### R-06b sync replay same execution_id: {json.loads(t4).get('execution_id') == json.loads(t5).get('execution_id')}\n"
    )
except Exception:
    pass
...
for i in range(40):
    c, _ = http(
        "POST", "/v1/execute", U, {"ask": f"burst-{i}", "execution_policy": {"async": True}}
    )
    codes[c] = codes.get(c, 0) + 1
OUT.append(f"### R-07 burst 40 async submits: status histogram {codes}\n")
time.sleep(2)
c, t = http("GET", "/v1/executions", U)
n = len(json.loads(t)["executions"])
st = {}
...
c, t = http("GET", "/healthz")
rec("R-09 healthz scope (process-local honesty)", c, t)
