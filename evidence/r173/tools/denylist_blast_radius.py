...
summary = {
    ...
    "forms": {
        "A_default_runtime": {
            ...
            "composed_by": "apps/composition/runtime.py::_source_reader (platform agent source_*)",
        },
        "B_hardened_agent_dev": {
            ...
            "collateral_by_ext": dict(
                collections.Counter(
                    pathlib.PurePosixPath(p).suffix or "<none>" for p, _ in collateral_B
                )
            ),
            "collateral_by_pattern": dict(
                collections.Counter(pat for _, pats in collateral_B for pat in pats)
            ),
        },
        "C_normalised_check": {
            "denied_total": len(denied_B) + len(denied_C_only),
            "normalisation_only_hits": len(denied_C_only),
            "note": "is_denied() = raw fnmatch then NFKC/invisible/ADS/trailing/casefold canon; "
            "on an ASCII tracked tree canonical == raw so C == B unless a tracked name "
            "carries case/unicode variance",
        },
    },
    "B_minus_A": len(denied_B) - len(denied_A),
    "patterns_with_zero_tracked_hits_B": sorted(
        p for p in DENIED_PATH_PATTERNS if pattern_hits_B[p] == 0
    ),
}
...
(OUT / "collateral_B.txt").write_text(
    "".join(f"{p}\t{','.join(pats)}\n" for p, pats in collateral_B)
)

print(
    json.dumps(
        {k: v for k, v in summary.items() if k != "patterns_with_zero_tracked_hits_B"}, indent=1
    )
)
print(
    "zero-hit hardened patterns:",
    len(summary["patterns_with_zero_tracked_hits_B"]),
    "of",
    len(DENIED_PATH_PATTERNS),
)
