...
| 2026-09-04 | D-03/D-04 (restarts #18, #19) | reset wiped un-uploaded docs commit c814a856; restored 824dce8a (fix) and re-applied docs as 520abe0e. DONE: RoutingRequest.credential_policy (+5/-1); ResourceSelector.complete() account-complete routes (+60/-0); ExecutionService account_credentials/audit seams + PROVIDER_ACCOUNT_USED per attempt (+66/-3) ⇒ round B 3/5; hermetic 2-account failover on ONE provider proven (7 tests); matrix row SUPPORTED VIA ACCOUNT POOL; boundary re-evaluation; IMPL-010; defect ledger FIXED (contract level). Composition wiring: OUT OF SCOPE (R168): budget — scheduled for R169. Live 2-account: NOT EVALUATED | 520abe0e | done |
| 2026-09-04 | GATE after D-03/D-04 | hermetic check_repo.sh RESULT: PASS — pytest 2769/0/0/64 (floor 2762→2769), budget round_a=4/5 round_b=3/5, NOT EVALUATED 2 (evidence/r168/D-03-04/check_repo_after.txt); credentialed run PASS 2775/0/0/58 (check_repo_after_credentialed.txt) | see commit | done |
| 2026-09-04 | D-11 (restarts #20, #21) | restored 57ca12bc (no loss); checkpoint: FAIL FIRST — non-admin on /v1/admin/* ⇒ 403 unchanged AND a PERMISSION_DENIED row in the CALLER's tenant (path, method, reason; no body echo); foreign project_id on /v1/execute ⇒ 404 unchanged AND a CROSS_TENANT_ACCESS_DENIED row in the actor's tenant (reference as given; target tenant unknowable by design, 20 §6); rows readable by that tenant's admin via GET /v1/admin/audit. Cross-tenant platform read: port forbids by design → OUT OF SCOPE (R168): design — scheduled for R169. Plan ≤2 changes (apps/api/app.py; apps/api/admin.py if needed) | 57ca12bc | in progress |
| 2026-09-04 | D-02 (restart #22) | restored fd85c4b6 (no loss; D-11 gate output lost with the reset → rerun in progress). checkpoint: FAIL FIRST — Groq normaliser on HTTP 400 `{"detail": "Model '…' is not allowed…"}` (no `error.code`) must book `model_unavailable` (non-indicting ⇒ failover permitted), detail text never crosses; other `detail`-only 400s stay `bad_request` (INV-4: no evidence they are non-indicting). Fix lives in providers/real/groq/adapter.py ⇒ budget-free (round B stays 4/5) | fd85c4b6 | in progress |
| 2026-09-04 | D-11 DONE | FIXED (partial): apps/api/app.py `_audit_denied` (+37/-0, round B 4/5) — PERMISSION_DENIED at the /v1/admin/* non-admin 403; CROSS_TENANT_ACCESS_DENIED at the unresolved project_id 404; actor's tenant; no body/secret/target echo; bodies unchanged; tenant admin reads via GET /v1/admin/audit. NOT FIXED (verbatim in ledger): CREDENTIAL_CREATED/REVOKED (no route), PROVIDER_ACCOUNT_USED at composition (R169), platform-wide audit read: OUT OF SCOPE (R168): design — scheduled for R169. IMPL-011; evidence/r168/D-11/ (5 artifacts) | fd85c4b6 | done |
| 2026-09-04 | GATE after D-11 | hermetic check_repo.sh RESULT: PASS — pytest 2773/0/0/64 (floor 2769→2773), budget round_a=4/5 round_b=4/5, NOT EVALUATED 2 (evidence/r168/D-11/check_repo_after.txt) | see commit | done |
