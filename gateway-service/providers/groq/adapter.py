"""Layer 2 — the Groq FACADE. The only layer the gateway sees.
...
(environment variable name documented in ``_upstream.GROQ_API_KEY_ENV``);
``context.credential_value`` is always None here and is never consulted.

ONE REQUEST -> ONE CANONICAL RESPONSE. ZERO retries (v1, ADR-0008).
...
from providers.groq._upstream import UpstreamReply, call_chat_completions
...
#: Canonical finish reasons (CONTRACT §1). Upstream values outside this set
#: (or absent) are mapped conservatively to "stop".
_FINISH_REASON_MAP = {
    "stop": "stop",
    "length": "length",
