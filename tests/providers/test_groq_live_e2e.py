"""LIVE full-stack end-to-end through the REAL Groq provider (T-IMPL-036).

This is the proof the 41 §49 closing rule demands: "End-to-end AI execution
is not considered complete until at least one real provider is implemented
and verified." The path exercised is the PRODUCTION composition — no fakes
anywhere in the chain:

    POST /v1/execute (httpx-ASGI)
      -> SimpleScoringRouter (real registries, groq registered ACTIVE)
      -> ExecutionService (usage reservation + settlement)
      -> GroqAdapter (real HTTPS call to api.groq.com)
      -> normalized response -> API result + usage block

Env-gated like tests/providers/test_groq_live.py: SKIPPED without
``GROQ_API_KEY``; the hermetic gates never touch the network. The key rides
...
registers groq with domain status ACTIVE deliberately — this test IS the
verification gate, and the registration is scoped to this test world.
"""
...
requires_live_key = pytest.mark.skipif(
    ...
    reason="GROQ_API_KEY not set — live e2e runs manually only (41 §49)",
)
...
def _live_world() -> tuple[Any, InMemoryUsageAccounting, Principal]:
    """Compose the production stack with the REAL Groq adapter."""
    tenant_id = uuid4()
...
class TestLiveEndToEnd:
    def test_execute_end_to_end_through_real_groq(self) -> None:
        ...
        assert os.environ["GROQ_API_KEY"] not in response.text
