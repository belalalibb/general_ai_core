...
def show(out, n=None) -> None:  # type: ignore[no-untyped-def]
    for s in out.report.steps[:n]:
        ob = s.observation
        print(
            "   step", s.index, ob.get("tool"), ob.get("status"),
...
scripted.script = [model_says(tool_call("ws_write", path="NOTE.md", content="hello\n")), model_says(final("done"))]
out = run("write NOTE.md", ["ws_write"]); show(out, 1); print("   NOTE.md exists:", exists("NOTE.md"))
...
ticket = ledger.issue(
    tenant_id=p.tenant_id, workspace=bundle.workspace_label,
    acts=[EngineeringAct.FS_WRITE, EngineeringAct.GIT_COMMIT, EngineeringAct.GIT_PUSH],
    issued_by=p.user_id, uses=3, note="R164 live proof",
)
tid = str(ticket.id); print("   ticket", tid[:8], "uses", ticket.uses_remaining)

print("== 4. POSITIVE: write -> status -> commit -> push -> log (real fs / real git / real subprocess)")
scripted.script = [
    model_says(tool_call("ws_write", path="NOTE.md", content="hello from agent\n", authorization_id=tid)),
    model_says(tool_call("git_status")),
    model_says(tool_call("git_commit", message="feat: NOTE.md written by the shared agent runtime", authorization_id=tid)),
...
out = run("add NOTE.md, commit, push", ["ws_write", "git_status", "git_commit", "git_push", "git_log"]); show(out)
print("   stop:", out.report.stop_reason, "| ok/failed:", out.report.summary["tool_calls_ok"], out.report.summary["tool_calls_failed"])
...
scripted.script = [model_says(tool_call("ws_write", path="AGAIN.md", content="x", authorization_id=tid)), model_says(final("done"))]
out = run("write again", ["ws_write"]); show(out, 1); print("   AGAIN.md exists:", exists("AGAIN.md"))
assert not exists("AGAIN.md")

print("== 6. Jail + command policy bind even WITH a valid ticket and exec permission")
t2 = ledger.issue(tenant_id=p.tenant_id, workspace=bundle.workspace_label,
                  acts=[EngineeringAct.FS_WRITE, EngineeringAct.CMD_RUN], issued_by=p.user_id, uses=5)
grant_engineering_writes(agent.firewall, p.tenant_id, frozenset({"workspace.exec"}))
scripted.script = [
    model_says(tool_call("ws_write", path="../escape.txt", content="x", authorization_id=str(t2.id))),
...
out = run("try escapes", ["ws_write", "ws_run"]); show(out, 4)
...
print("   escape.txt outside jail exists:", escaped, "| .env exists:", exists(".env"), "| t2 uses left:", t2_left)
assert not escaped and not exists(".env") and t2_left == 4

print("== 7. Audit trail (surface=engineering_authorization)")
ev = [e.details for e in ledger._audit.read(p.tenant_id) if e.details.get("surface") == "engineering_authorization"]  # noqa: SLF001
print("   ", dict(Counter(str(d.get("act")) for d in ev)),
      "| refusal reasons:", sorted({str(d.get("reason")) for d in ev if d.get("act") == "refuse"}))
print("PROOF OK")
