...
**R197 note (UI closure):** the Workbench (`ui/app/`) now offers a template picker fed ONLY by `GET /v1/templates` (options = server rows keyed by `ref`) and sends `execution_strategy {mode:"template", template_id: ref}` only when a template is chosen; `/v1/templates/{ref}` is not consumed. The three R195 admin actions (`register_repo_binding` / `grant_remote_trust` / `revoke_remote_trust`) need NO console change: the R180 discovery-driven change form offers every action of an active area (TOOLS is active) with its declared `fields` hint — pinned by `tests/ui/test_r197_admin_actions_discovered.py`. `ui/admin` stays at 73 = N0; the Workbench is guarded by `ui_workbench_static_check` (ceiling 22) for the first time.
