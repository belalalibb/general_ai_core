...
RESUME_TOKEN:
PROJECT|R042|PHASE_2_IMPLEMENTATION|MVP_PHASE_5_SLICE_3_DONE|T_IMPL_023_VERIFIED|NEXT_MVP_PHASE_5_USAGE_SETTLEMENT

LAST_VERIFIED_LOCAL_COMMIT:
VERIFY_WITH_GIT_REV_PARSE_HEAD (R042 session: the T-IMPL-023 executing session was interrupted at the state-checkpoint step AFTER all gates had passed; the auto-uploader synced the code as per-file commits. This session applied the recovery rule — verified ALL artifacts from filesystem (apps/api/{__init__,app,errors,store}.py + tests/api/test_execute_api.py present and tracked; fastapi pin + 8th import-linter contract present in pyproject.toml), reinstalled the full pinned dep set after ANOTHER sandbox reset (fastapi/pytest/mypy/ruff/import-linter all gone), untracked re-tracked tool caches again, re-ran ALL gates green (430 tests; check_repo.sh PASS), then wrote this checkpoint. Trust HEAD + filesystem + green gates over old hashes.)

LAST_VERIFIED_STATE_TASK:
T-IMPL-023 — MVP Phase 5 slice 3: API surface in apps/api/ (POST /v1/execute + GET /v1/executions/{id}, FastAPI per ADR-0001; dependency pin + import-linter contract landed in the same change set), completed + VERIFIED with green gates at this checkpoint (R042).
...
```text
LAST_VERIFIED_TASK:
T-IMPL-023 — API surface: POST /v1/execute + GET /v1/executions/{id} at the apps/ composition root (MVP Phase 5 slice 3, R042)

LAST_VERIFIED_TASK_COMMIT:
VERIFY_WITH_GIT_REV_PARSE_HEAD (auto-uploader history rewrites make stored hashes unreliable; content verified against filesystem + green gates at R042)
...
MVP Phase 5 — Routing + Execution MVP (41 §44): slice 1 (router simple scoring) DONE at R040; slice 2 (execution service single + pipeline) DONE at R041; slice 3 (API surface POST /v1/execute + status endpoint) DONE at R042. Remaining 41 §44 deliverable per the Phase 5 slicing decision below: T-IMPL-024 usage reservation/settlement — then the Phase 5 exit evaluation.

NEXT_TASK:
T-IMPL-024 — MVP Phase 5 slice 4: usage reservation/settlement in core/usage/ (41 §44; 03 §7 Usage/CostSnapshot records; 10 §3 usage block). Reservation BEFORE execution (estimate against tenant budget/entitlement), settlement AFTER from RAW provider usage already carried on the T-IMPL-022 cost_snapshot (settlement=pending_usage_service marker was recorded exactly for this slice). Port + in-memory fake pattern (no real billing/network); wire into ExecutionService seams and surface the 10 §3 usage block from the API (currently usage=None — recorded at R042 as this slice's job). Hermetic tests; deny-by-default on missing entitlement data must be an explicit decision, not an accident.

NEXT_TASK_OBJECTIVE:
Executions reserve estimated usage before provider work, settle actual usage from normalized provider usage after, expose the settled usage block on the API response, and fail loudly (never silently) when reservation cannot be established.

NEXT_TASK_NOTE:
Constraints carried forward: hermetic gates only (no network), credential handling stays opaque-reference-only (20 §5), unknown capability => DENY (30 §7), templates never routable/executable (31 §4/§10), 41 §49 NOT-CLAIMED rule still binds. After T-IMPL-024 verifies, run the MVP Phase 5 exit evaluation against 41 §44 before ANY Phase 6 work (R036 directive interpretation: verified exit checkpoint first).

NEXT_TASK_AUTHORIZED:
YES (USER DIRECTIVE 2026-08-26 R036 continuous-execution directive; Phase 5 is open, slices 1-3 verified at this R042 checkpoint).
