...
SUPERSEDES:
  final_docs_v2/09_SKILL_TOOL_SPEC.md
MIGRATION_TYPE: CARRY (verbatim)
DECISION_PRESERVATION: Skill/Tool definitions, manifests, import lifecycle,
tool locations, client runtime, Capability Firewall check, GitHub tool
permissions, forbidden list, tests, and provider-agent tool use carried
unchanged. No decision changed.
RELATED_AUTHORITY:
  Security / Capability Firewall authority: v2 10 until T-DOC-009, then final_docs_v3/20_SECURITY_THREAT_MODEL.md
  Provider-agent tool mediation:            final_docs_v3/12_EXECUTION_GRAPH_AND_AGENT_MODE.md
```
...
|---|---|
| 1–11 (definitions, skill manifest, skill import lifecycle, tool manifest, tool locations, client runtime, Capability Firewall check, GitHub tool permissions, forbidden list, tests, provider-agent tool use) | v2 09 §1–11, carried verbatim |

CARRY migration: content unchanged; only the V3 authority header and this
traceability section were added. Capability Firewall rules and the forbidden
list were not modified.
