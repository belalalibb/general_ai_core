...
SUPERSEDES:
  final_docs_v2/04_API_CONTRACTS.md
MIGRATION_TYPE: CARRY (verbatim)
DECISION_PRESERVATION: All endpoints, request/response schemas, error codes,
model policy types (auto / tier / explicit_model / explicit_models /
agent_node_mapping), idempotency, streaming, and webhook contracts carried
unchanged. No decision changed.
RELATED_AUTHORITY:
  Routing behavior behind these contracts: final_docs_v3/11_MODEL_ROUTING_AND_MODEL_CONTROL.md
  Execution strategies referenced here:    final_docs_v3/12_EXECUTION_GRAPH_AND_AGENT_MODE.md
```
...
|---|---|
| 1–13 (principles, /v1/execute, responses, status, models/skills/usage endpoints, unified errors, idempotency, streaming, webhooks, advanced model control contract incl. all 5 policy types and model control error codes) | v2 04 §1–13, carried verbatim |

CARRY migration: content unchanged; only the V3 authority header and this
traceability section were added. No endpoint, schema, policy type, or error
code was modified.
