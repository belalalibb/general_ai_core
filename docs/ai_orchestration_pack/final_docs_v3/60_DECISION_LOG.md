...
### R191-DEC-01 — R191 opened by DECLARATION: General Agent + extensible Skills/Strategy/Template foundation + App Factory foundation (first vertical slice) (2026-09-20)

**Operator text (verbatim anchors):** "This round is NOT 'build a hardcoded App Factory'" · "Agent != Model != Provider != Skill != Template != App Factory" · "Do NOT create an AppFactory-specific Agent Engine / Router / Model Manager / Provider Manager / second execution engine" · "Templates MUST be data/configuration, NOT hardcoded code branches" · "Model identity ≠ Provider resource capacity" · "If R191 genuinely requires a frozen contract SHAPE change: STOP before implementing that expansion" · "Do NOT start [an unapproved next round]."

**Baseline verified (Part 1):** `git status` clean; `HEAD = origin/main = dc1c62de`; open PRs 0; `PROJECT_EXECUTION_STATE.md` R190 pointer, R190-DEC-02, `R190_HANDOFF.md` §7, `evidence/r190_state_ledger.md` row 13, `CONTRACT_FREEZE_RECORD_R190_UPDATE.md` all agree (R190 CLOSED, App Factory NOT started, `min_passed=3779`, `not_evaluated=1`). No disagreement between records and Git.

**Discovery (Part 22 — read on the tree, reuse-first):**

| Existing | Where | R191 use |
|---|---|---|
| `ExecutionStrategySpec` (`mode auto/custom/template`, `StrategyStage{key,kind,role,instruction,model_policy,depends_on,payload}`, DAG waves, MAX_STAGES 16 / MAX_PARALLEL 4) | `core/contracts/execution_strategy.py` (FROZEN shape) | THE primary customization unit (Part 4). Reused as-is; templates PRODUCE it. |
| `StrategyExecutor(router, execution, templates: Mapping[str, ExecutionStrategySpec])`; `resolve()` refuses unknown template ids; per-stage `RoutingRequest(model_policy=stage.model_policy, required_capabilities=…)` | `core/execution/strategy.py` | The ONE strategy engine. Unchanged; the new registry exposes a `Mapping` view. |
| `/v1/execute` accepts `execution_strategy`, `skills`, `requirements`, `model_policy`; admits skills against `SkillRegistry.list_selectable()`; `SkillResolver` auto-selection | `apps/api/app.py:878-1060`, `core/skills/resolver.py` | Served surface unchanged; the same admission rule is reused by the General Agent. |
| `Skill`/`SkillManifest` (versioned, `capabilities`, `requires_tools`, `runtime.compatible_roles`, provenance/import) ; `SkillRegistry` | `core/contracts/skills.py`, `core/roles/registry.py:92` | Skills stay first-class (Part 7); `manifest.capabilities` + `requires_tools` are the "skill requirements" that flow into capability requirements (Part 8). No second registry. |
| Router: `bindings_for_model`, per-(provider, model) `ResourceSignalBoard`, same-model fallback (R188/R190) | `core/routing/router.py`, `core/routing/capacity.py` | Part 9/10 guaranteed by the existing router — VERIFIED by tests, not re-implemented. |
| Provider-native agent contracts (`AgentCapabilityType.PROVIDER_AGENT`, `AgentRuntimeBinding.provider_managed`, `core/contracts/provider_agent.py`) | `core/contracts/domain.py:152-190` | Part 13 distinction already exists; the General Agent treats such a model as a routed stage, never as an authority. |
| GitHub transport (`GitHubRestTransport`: fetch/status/commit/push/PR; httpx, no shell; token per call, never stored) + `RepoBinding` (tenant-scoped, `RemoteTrustPort`) | `apps/agent_dev/github_transport.py`, `git_tools.py`, `core/contracts/repo_binding.py` | Part 15/16 GitHub connection reuses `RepoBinding` + trust; a READ-ONLY tree inspection is missing → new `apps/agent_dev/project_inspector.py` (same httpx posture). |
| **Absent:** any template abstraction (templates are a bare dict passed at composition; nothing is passed today — `strategy_templates=None`), template versioning/origin/override, an agent-capability registry (only `CapabilityFirewall` = security and `CapabilityProposal` = admin exist), a General Agent planner, App Factory. | — | Built in R191 as NEW modules (not duplicates — repository evidence: no counterpart exists). |

**Design (Parts 2–16), additive only:**
- `core/contracts/agent_template.py` (NEW): `TemplateOrigin{system,imported,user,workspace}`, `TemplateStatus{active,disabled,archived}`, `StrategyTemplate{id, version, name, origin, status, description, strategy: ExecutionStrategySpec, skills: [skill ids], required_capabilities, tags, metadata}`, `TemplateOverride{model_policy_all, stage_model_policies{stage_key→NodeModelPolicy}, add_skills, drop_stages? NO — additive only: add_skills + model overrides + max_parallel}`, `AgentPlan{strategy, template_ref, skills, required_capabilities, overrides_applied, capability, notes}`. Declarative, UI-manageable configuration (Part 6) — not a served route this round.
- `core/execution/templates.py` (NEW): `TemplateRegistry` — `register(template)` refuses same `(id, version)` twice and refuses a non-system origin re-registering a system id at the same version (Q: older versions never mutate); `get(id, version=None)` → latest active; `list(origin=…)`; `apply_overrides(template, override) → ExecutionStrategySpec` (explicit, inspectable: returns the runtime spec + an `overrides_applied` record); `as_strategy_mapping()` → `Mapping[str, ExecutionStrategySpec]` keyed `"{id}"` (latest) and `"{id}@{version}"` so the EXISTING `StrategyExecutor(templates=…)` consumes it unchanged.
- `core/agent/general.py` (NEW): `AgentCapabilityRegistry` (Define→Contract→Register→Discover→Compose: a capability is `{name, version, description, contributes(request, context) → CapabilityContribution{strategy?, skills, required_capabilities, notes}}`); `GeneralAgent(templates, skills: SkillRegistry, capabilities, executor: StrategyExecutor)` with `plan(GeneralAgentRequest{ask, capability?, template_ref?, template_override?, strategy?, skills, requirements}) → AgentPlan` and `execute(plan, …)` → `StrategyReport` through the ONE executor. Skill requirements → `required_capabilities` union; unknown/non-selectable skill ⇒ loud refusal (same rule as `/v1/execute`). No provider-name branching; no task-type enum.
- `core/agent/app_factory.py` (NEW): `AppFactoryCapability` (a registry entry, NOT an engine): `inventory(ProjectInspectorPort, binding) → ProjectInventory`; `qevion_inventory(models, skills, capabilities)`; `contributes()` → a data-driven default template `app_factory.plan@1` (stages: inventory-summary → architecture-plan → review) that the General Agent runs like any other strategy; output = `ApplicationPlan` (records). Generation deferred to a later slice (Part 16).
- `apps/agent_dev/project_inspector.py` (NEW): `GitHubProjectInspector(ProjectInspectorPort)` — `GET /repos/{o}/{r}/git/trees/{sha}?recursive=1` over httpx with the R172 transport posture (token per call, never stored, `RemoteTrustPort` respected by the caller); returns a bounded `ProjectInventory` (paths, languages by extension, manifests detected). Tests use `httpx.MockTransport` — ZERO live calls.
- Headroom: `apps/composition/runtime.py` → `create_app(strategy_templates=registry.as_strategy_mapping())` for built-in system templates (only if the served path is exercised in tests; otherwise deferred and recorded).

**Ceiling:** `round_r191` = 6 (see manifest). **Not touched:** `core/execution/strategy.py`, `core/routing/*`, `core/execution/service.py`, `apps/api/app.py`, all 15 frozen modules, `infrastructure/`, `ui/`, `apps/admin_agent/*`.

**Contract-shape stance (Part 25):** all of the above lives in NEW modules; `contract_freeze_derive.py --check` must MATCH at the gate. A served template route (`/v1/templates`) would add to `served_routes_v1` ⇒ SHAPE change ⇒ **PROPOSAL P-R191-01** (recorded in R191-DEC-02), not implemented.

**RED-first proof map (Part 23):** A–F, K, L, M, N, O, Q in `tests/agent/test_r191_general_agent.py` + `tests/execution/test_r191_templates.py`; G–J re-verified via `tests/routing/test_r188_capacity_signals.py` + `tests/providers/test_r190_logical_model_identity.py` (unchanged, must stay GREEN) plus a General-Agent-driven same-model fallback test; App Factory slice in `tests/agent/test_r191_app_factory_slice.py` (MockTransport); P = gate; R = durability truth: the registry is in-memory (no persistence this round — recorded, not pretended).

**Exit conditions:** RED → GREEN; regression; fresh-clone gate PASS + gateway 194; D-6 ratchet; freeze `--check` MATCHES; PR by merge commit; post-merge gate; R191-DEC-02; stop on main — App Factory generation NOT started.

### R191-DEC-02 — R191 CLOSED: General Agent + Template/Skill foundation + App Factory first vertical slice DELIVERED (5 new production files, contract shape unchanged); gate of record 3809/0/0/64; PR #40 merged (2026-09-20)

**Operator text (verbatim anchors):** "Agent ≠ Model ≠ Provider ≠ Skill ≠ Template ≠ App Factory" · "Execution Strategy is the primary customization unit" · "Templates MUST be data" · "Core = engine / Provider = fuel" · "App Factory is a capability of the General Agent" · "do NOT create AppFactory-specific engine/router/model manager/provider manager or second execution engine, second registries, TemplateExecutor" · "declare production ceiling before first production commit" · "then stop cleanly (no App Factory generation)".

**Deliverables (1–16):**

1. **General Agent** — `core/agent/general.py` `GeneralAgent.plan()`/`execute()`: one HOW per request (strategy | template_ref | capability; two at once refused "exactly one"); execution is a thin call into the ONE `StrategyExecutor`. Proof A/L/N + two-structures test.
2. **Agent ≠ provider-native agent** — `"general"` is NOT an `AgentCapabilityType` member (provider_agent/managed_assistant remain the provider-native types); `GeneralAgent.PLATFORM_AUTHORITY` names what the agent never owns (authorization, firewall, tool approval, tenant isolation, usage, routing, evaluation, audit, final response policy). Proof M.
3. **Templates are DATA** — `core/contracts/agent_template.py` `StrategyTemplate` (id, version, name, origin, status, concrete custom `ExecutionStrategySpec`, skills, required_capabilities, tags, metadata); JSON round-trip proven; mode≠custom refused at construction.
4. **Template registry** — `core/execution/templates.py` `TemplateRegistry`: immutable (id,version); system-owned id reservation (user/imported cannot shadow); latest-ACTIVE lookup; disabled → listable, not resolvable; `resolve_ref("id" | "id@version")`. Proof B/D/Q.
5. **Existing executor consumes templates unchanged** — `as_strategy_mapping()` is a LIVE `Mapping[str, ExecutionStrategySpec]` handed to the EXISTING `StrategyExecutor(templates=...)`; later registrations are visible; unknown → existing `UnknownTemplate`. No `TemplateExecutor`.
6. **Overrides explicit + inspectable** — `TemplateOverride` (per-stage model policy, `model_policy_all`, add_skills, add_required_capabilities, max_parallel) → `apply_override` returns `OverrideRecord`s carried in `AgentPlan.overrides_applied`; unknown stage → `OverrideRejected`. Precedence is the operator chain (Base Template → Agent Config → User Override → Runtime Strategy) with per-stage over all, recorded — no hidden rule. Proof C/K.
7. **Skills first-class, one admission rule** — skills admitted by the EXISTING `SkillRegistry.list_selectable()` (ACTIVE+LOCAL) by manifest id; non-selectable/unknown → `PlanRefused("skill is not selectable: …")` exactly like `/v1/execute`. Proof O.
8. **Skill requirements → capability requirements** — manifest capabilities union into `AgentPlan.required_capabilities` (sorted, deduplicated) and flow to `executor.execute(required_capabilities=…)` → the EXISTING router filter. Proof E/F/N.
9. **Model customization A–E through the agent** — AUTO (stage `model_policy=None`), one logical model (`ExplicitModelPolicy model_id`), per-stage (`stage_model_policies`), explicit provider+model (`provider_id`), same model many providers. Proof L/K/J/G-I.
10. **Part 10 — Model identity ≠ Provider capacity** — re-verified on the UNCHANGED router/execution through the agent: rate-limited provider → other binding of the SAME `model_id` (2 attempts, different providers); cooled provider excluded on the next decision (per-binding signal). Proof G/H/I.
11. **Capability registry** — `AgentCapabilityRegistry` (register/get/list; duplicate → ValueError); contributors return `CapabilityContribution` (strategy | template_ref, skills, required_capabilities, notes); unknown → "unknown capability". Proof N.
12. **App Factory = capability, not engine** — `core/agent/app_factory.py` `AppFactoryCapability.as_capability()` registered in the ONE registry; contributes the DATA template `app_factory.plan@1` (inventory-summary → architecture-plan → review); resolves the inventory from `context.project_inventory` or a token-free `ProjectInspectorPort`; refuses loudly when neither exists. `qevion_inventory()` READS the existing model/skill/capability registries (no copy).
13. **First vertical slice end-to-end (generation deferred)** — GitHub connection → `GitHubProjectInspector` (2 read-only GETs, bounded inventory) → `ProjectInventory` → `QevionInventory` → template selection → model policy stated (`auto` / `explicit-per-stage` / mixed) → provider routing posture `same-model-fallback-preferred` → `ApplicationPlan` (`generation_deferred: Literal[True]`). Executed through the ONE executor: 3 stages, all on the ONE model. `tests/agent/test_r191_app_factory_slice.py` 11/11, `httpx.MockTransport`, ZERO live calls; token never in inventory/plan/exception text.
14. **Contract shape — NONE changed.** `contract_freeze_derive.py --check` MATCHES on branch head and main; served `/v1/templates` recorded as **PROPOSAL P-R191-01** (not executed). `evidence/r191/CONTRACT_FREEZE_RECORD_R191_UPDATE.md` (6 deferred items with seam/trigger/owner/proof).
15. **Governance** — declaration before code (R191-DEC-01; manifest `round_r191` ceiling 6 declared before the first production commit; 5/6 used; R191-C headroom deliberately unused); RED committed before production for every module (`evidence/r191/red_*.txt` → `green_*.txt`); 3 TEST-only fixes during GREEN (`SkillStatus.DRAFT` never existed; requirement satisfiability; override expressed explicitly) — zero production changes needed after RED; mypy strict clean; ruff clean; regression 2721/13/0 incl. R188/R189/R190 suites.
16. **Gates + merge** — gate of record fresh clone `f608c669` PASS **3809/0/0/64** + gateway **194**; D-6 ratchet `min_passed` 3779 → 3809 (+30 = 8+11+11); PR #40 merged by merge commit → `main 7c29ac33` (preconditions clean; protection 404; 0 statuses/check-runs/workflows); post-merge fresh clone `7c29ac33` PASS **3809/0/0/64** + gateway **194**; manifest `last_measured` → 7c29ac33.

**Exit criteria:** additive-only ✔ (`git diff --numstat dc1c62de 7c29ac33 -- core apps` = 5 additions, 0 modifications) · no second engine/router/registry/TemplateExecutor ✔ · templates data ✔ · UI untouched ✔ · skills first-class via existing registry ✔ · Part 10 ✔ · override chain explicit ✔ · provider-native ≠ platform agent ✔ · slice to application plan, generation NOT started ✔ · freeze SHAPE unchanged ✔ · deferred items not reopened ✔ · ceiling declared first, N+1 not reached ✔.

**Exact next continuation point.** `main = 7c29ac33` + this records-only closure PR. Next session starts from main, reads `PROJECT_EXECUTION_STATE.md` R191 pointer and the last row of `evidence/r191_state_ledger.md`, and does NOT open App Factory generation (or P-R191-01) without an operator declaration in this log.

### R192-DEC-01 — R192 CLOSED: claim verification H1–H11 against code + V3 authorities; two proven gaps closed additively (governed project inspection; layering pin); four operator decisions recorded; gate of record 3826/0/0/64; PR #42 merged (2026-09-20)

**Operator text (verbatim anchors):** "Treat every architectural statement … as a HYPOTHESIS, NOT AS FACT" · "NO PRODUCTION WRITES ARE ALLOWED DURING THIS INITIAL REVIEW PASS" · "Do NOT create a second runtime" · "Use the existing Secret Manager / credential-ref authority. Do not create a new secret system." · "Do not invent a new Git client." · "VERIFY → RECONCILE → PROVE → MINIMALLY CLOSE → REGRESS → RECORD."

**Part 14 — final verification:**

- **A. Verified TRUE:** H1 (GeneralAgent is an unserved library layer; zero production consumers; separation intentional per 12 §2); H2 (StrategyExecutor = model stages via `execute_single`; AgentRuntime = tool loop via ToolCallGate→ToolExecutor); H4 (TemplateRegistry in-memory, process-lifetime, origin≠ownership, no served route); H5 (inspector had no tenant/trust/credential chain); H6 (REST-git path uncomposed by IMPL-024 owner decision); H7 (lifecycle unwired); H9 (flexibility preserved, nothing hardcoded); H10 (security chain intact; `AgentToolSpec.binding()` never self-approves). Evidence: `evidence/r192/REVIEW_IMPACT_MATRIX.md`.
- **B. Disproved:** "a missing engine is needed for generation" (H2 — AgentRuntime + engineering tools + GitToolset own every tool-class stage); "skill instructions are silently dropped at runtime" (H3 — the frozen `SkillManifest` has no instruction body; `{id,name,version}` + `skill_tools` + capability requirements are the recorded DATA-only posture, `app.py:1284`); "GeneralAgent duplicates a runtime" (H1).
- **C. Already implemented (reused, not rebuilt):** per-call idempotency (`ExecutionService`, StrategyExecutor per-stage keys, `/v1/execute` index, outbox at-least-once) — H8; approval gates (ToolCallGate BEFORE_ACTION + IMPL-023 payload binding) — H10; per-stage evaluation intake (`EvaluationPolicy.judge(tenant, execution_id, output)`) — H11; RepoBinding/remote-trust/secret chain in `GitToolset` — H5 source of truth.
- **D. Actually changed:** `core/agent/app_factory.py` (+44/−3): `BindingInspectorPort`, `AppFactoryCapability(binding_inspector=None)`, `context.binding_id` path → `PlanRefused` carrying the typed `GitRefusalCode` label. `apps/agent_dev/project_inspector.py` (+81/−2): `BoundProjectInspector`. Tests: `tests/agent_dev/test_r192_bound_inspector.py` (11), `tests/agent/test_r192_layering_guard.py` (6).
- **E. Why each file was necessary:** `project_inspector.py` — the only place a governed adaptor can live beside the transport primitive without Core importing apps; `app_factory.py` — Core must accept a binding id (not URL+token) so identity/trust/credential stay outside Core. No third file needed; ceiling 2 declared first (matrix, 2e5202a5), used 2.
- **F. Contract shape impact:** NONE — `contract_freeze_derive.py --check` MATCHES at a11b7088 and on main. No route added.
- **G. Security impact:** positive — inspection now refuses untrusted remotes BEFORE any credential resolve (fail closed on registry fault), resolves the secret at the last moment, never stores it; token proven absent from inventory, notes, repr, and exception text.
- **H. Tenancy impact:** enforced — `RepoBindingRegistry.get(tenant_id=)`; foreign tenant → `binding_tenant_mismatch` (never "unknown").
- **I. Persistence impact:** NONE.
- **J. Authorities reused:** RepoBindingRegistry, RemoteTrustPort, SecretManagerPort, GitRefusalCode/BindingLookupRefused, GitHubProjectInspector, parse_github_remote, StrategyExecutor, AgentRuntime (unchanged), SkillRegistry (unchanged), TemplateRegistry (unchanged).
- **K. Tests:** RED `ImportError: BoundProjectInspector` at 40b5a8c1 (`evidence/r192/red_g1_g2.txt`) → GREEN 17/17 (`green_g1_g2.txt`); R191 slice 11/11 unchanged.
- **L. Regression:** 2951 passed / 15 skipped / 0 failed at 1eab7790 (`evidence/r192/regression_1eab7790.txt`), incl. R188/R190/R191 suites.
- **M. Static:** mypy strict clean (3 modules); ruff check/format clean; budget guards 33/33.
- **N. Remaining deferred (re-checked, still deferred):** App Factory generation wiring (needs P-R192-04; remote test runner has no primitive); stage-level resume (12 §9 → durable runtime); strategy-output evaluation; composition of built-in templates (R191-C); template durability.
- **O. Operator decisions required:** **P-R191-01** served `/v1/templates` (SHAPE; still required); **P-R192-02** skill instruction channel (frozen `SkillManifest`; options a/b/c); **P-R192-03** template ownership axis (tenant/workspace/user undefined in V3); **P-R192-04** compose `dev_bindings` + `GitToolset` (+`BoundProjectInspector`) into production (IMPL-024 owner item; needs binding-store + trust-store sources). Details: `evidence/r192/CONTRACT_FREEZE_RECORD_R192_UPDATE.md` §3.
...
### R194-DEC-01 — R194 OPENED (operator "APPROVE R194. Execute R194 exactly as proposed."): Hardening + Composition Closure — non-architectural; ceiling 3; N-5 checkpoints withdrawn by repository finding (2026-09-20)

**Operator text (verbatim anchors):** "APPROVE R194. Execute R194 exactly as proposed. Preserve the shared multi-tenant architecture and all existing security, approval, tenancy, and contract invariants. Do not pre-approve AD-1, AD-2, R195, R196, or any other future decision. Stop at the next operator decision gate and wait for approval."

**Baseline:** `main = b5c46561` (R193 closed, PR #45), clean, open PRs 0. Branch `genspark_ai_developer_r194`.

**Decision:** execute the R194 proposal from the closure audit (Part 12) with the following slice, declared BEFORE any production commit:
- **R194-A — `apps/api/app.py`:** (N-1) one `@app.middleware("http")` that sets `X-Content-Type-Options: nosniff`, `X-Frame-Options: DENY`, `Referrer-Policy: no-referrer`, `Content-Security-Policy: default-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline'; img-src 'self' data:; connect-src 'self'; frame-ancestors 'none'; base-uri 'self'; form-action 'self'` on every response (the static UIs load exactly one external `<script src>` each and no inline scripts — measured; `style-src 'unsafe-inline'` is required by two `style=` attributes in `ui/admin/index.html` and one `.style.` write, so the UI trees stay frozen), and `Strict-Transport-Security` ONLY when `create_app(hsts=True)`; (N-2) `create_app(openapi_public: bool = True)` — when `False`, `GET /openapi.json` and `GET /redoc` require an admin bearer (401 tokenless, 403 non-admin — the same posture as `/v1/admin/*`). `app.openapi()` in-process (freeze derivation, `apps.cli check`, tests) is unaffected.
- **R194-B — `apps/composition/runtime.py` + `apps/main.py`:** (N-3) ONE `webhook_subscriptions: dict[UUID, list[WebhookSubscription]]` created at the root and passed to BOTH `create_app(webhook_subscriptions=)` and `ExecutionMessageHandler(outbox=, subscriptions=)` — today the handler receives neither, so `execution.succeeded/failed` are never staged in production even though `/v1/webhooks` accepts registrations; a second `Worker` (existing class) on `WEBHOOK_STREAM` with the existing `WebhookDeliveryHandler` and an `httpx.AsyncClient` sender composed at the root (ADR-0008: core never does outbound HTTP), `follow_redirects=False` (the SSRF admission is re-judged at delivery on the registered URL only), timeout `WEBHOOK_TIMEOUT_SECONDS` (default 10), non-2xx ⇒ exception ⇒ Worker retry/dead-letter posture unchanged (40 §4.7); `RuntimeProfile.webhook_worker`; `apps/main.py` drives it beside the execute worker in the same lifespan and cancels it in the same order. Composition posture: `hsts = env HSTS == "1"`; `openapi_public = env OPENAPI_PUBLIC` if set else `not durable` (in-memory dev profile stays public — the browser suites and `DEV_DEMO_PRINCIPAL` flows are unchanged; the durable profile is gated by default).
- **Ceiling 3** (`apps/api/app.py`, `apps/composition/runtime.py`, `apps/main.py`); a fourth production file ⇒ STOP. Core untouched.

**Finding R194-F1 (N-5 withdrawn):** the closure audit proposed composing `CheckpointManager`. Measured: the only seam is `build_dev_surface(checkpoints=)` in `apps/agent_dev/surface.py`, and `build_dev_surface` has ZERO production callers (`apps/composition`, `apps/api`, `apps/main`, `apps/cli`: 0). The production write tool `ws_write` (`core/engineering/tools.py`) carries no checkpoint hook. Composing checkpoints therefore means either composing the whole dev surface (a choice of WHICH surface serves tenants — overlaps AD-1/AD-7) or a Core change to `core/engineering/tools.py` — both are operator decisions. N-5 is recorded here and presented at the R194 closure gate; NOT implemented in R194.

**Invariants preserved (stated, to be proven by tests):** shared multi-tenant — subscriptions keyed by `tenant_id`, `stage_execution_event` receives only the owning tenant's rows, foreign tenants receive nothing; no admin-only execution path; approval/tenancy/provider/router/executor/registry boundaries untouched; contract SHAPE unchanged (`contract_freeze_derive.py --check` must MATCH; no `served_routes_v1` delta — `/openapi.json` and `/redoc` are outside the frozen set); unified error envelopes reused for the gate.

**RED-first proof plan:** `tests/security/test_r194_security_headers.py` (headers on `/healthz`, a `/v1/*` JSON route, an SSE route and a mounted static UI path; HSTS absent by default / present with `hsts=True`; `openapi_public=False` ⇒ 401 tokenless / 403 non-admin / 200 admin for both `/openapi.json` and `/redoc`; default profile public; durable-default posture from `runtime.py` computed value) · `tests/composition/test_r194_webhook_delivery.py` (default profile exposes `webhook_worker`; register subscription via `POST /v1/webhooks` → async `/v1/execute` → drive relay + execute worker + webhook worker → `MockTransport` receives `execution.queued` AND the terminal event for that execution id, body tenant = caller, POST to the registered URL only; a second tenant's subscription receives nothing; 5xx from the receiver ⇒ message not acked (retry posture) ; redirects not followed) · `tests/...` for `apps/main.py` lifespan (webhook task created and cancelled — hermetic via injected profile).

**Exit:** GREEN, regression incl. `tests/api/test_webhook_producers_v6.py`, `tests/security`, `tests/ui`; freeze `--check` MATCHES; mypy strict; ruff; fresh-clone gate + gateway 194; D-6 ratchet at the gate of record; PR by merge commit; post-merge gate; R194-DEC-02; then STOP at the operator decision gate (N-5 / AD-1…AD-7 remain unapproved).

### R194-DEC-02 — R194 CLOSED: hardening headers + admin-gated OpenAPI documents + webhook delivery composed (3 production files, contract shape unchanged); gate of record 3852/0/0/64; PR #46 merged; N-5 withdrawn (R194-F1) → operator decision gate (2026-09-20)

**Operator text (verbatim anchors):** "APPROVE R194. Execute R194 exactly as proposed. Preserve the shared multi-tenant architecture and all existing security, approval, tenancy, and contract invariants. Do not pre-approve AD-1, AD-2, R195, R196, or any other future decision. Stop at the next operator decision gate and wait for approval."

**Part 14 — final verification:**

- **A. Verified TRUE:** no hardening headers existed in `apps/`; `/openapi.json` + `/redoc` were unauthenticated; `WebhookDeliveryHandler` had 0 composition callers AND `ExecutionMessageHandler` was composed without `outbox`/`subscriptions`, so terminal webhook events were never staged in production although `/v1/webhooks` accepted registrations (ledger row 2).
- **B. Disproved / withdrawn:** the audit's N-5 "compose `CheckpointManager`" — finding **R194-F1**: `build_dev_surface(checkpoints=)` is the only seam and has 0 production callers; `ws_write` (`core/engineering/tools.py`) has no checkpoint hook. Composing it means choosing which dev surface serves tenants (overlaps AD-1/AD-7) or a Core change — an operator decision, NOT executed.
- **C. Already implemented (reused, not rebuilt):** `bearer_token`/`unauthenticated`/`error_response`, the AA-1 `_principal` resolver, D-11 `PERMISSION_DENIED` audit, `Worker`, `OutboxRelay`, `WEBHOOK_STREAM`, `WebhookDeliveryHandler`, `stage_execution_event` + `validate_webhook_url` (FIX-07 SSRF), `ExecutionMessageHandler(outbox=, subscriptions=)` and `create_app(webhook_subscriptions=)` seams, `httpx` at the composition root (ADR-0008).
- **D. Actually changed (3 files = ceiling 3, Core untouched):** `apps/api/app.py` (+69) — `create_app(hsts=False, openapi_public=True)`, ONE outermost header middleware, admin-gated documents; `apps/composition/runtime.py` (+85) — shared tenant-scoped subscription map, `build_webhook_sender`, `webhook_worker`, `openapi_public_from_env`, `HSTS` posture; `apps/main.py` (+16/−4) — lifespan drives `webhook-worker`. Fix within file 1: header middleware registered LAST (outermost) so admission short-circuits carry it.
- **E. Why each file was necessary:** `app.py` owns the FastAPI object and the only admission resolver; `runtime.py` is the only production composition root; `main.py` owns the process lifecycle (the profile is injection-only).
- **F. Contract shape impact:** NONE — `contract_freeze_derive.py --check` MATCHES at 7ee14495 and on `main 4f494344`; `served_routes_v1` = 44 unchanged. Headers are additive response metadata; `/openapi.json` + `/redoc` are outside the frozen set and only become stricter (401/403 unified envelope) on the durable profile or `OPENAPI_PUBLIC=0`.
- **G. Security impact:** positive — nosniff / frame DENY / no-referrer / self-only CSP on every response (incl. static UIs, SSE, error envelopes); HSTS opt-in; API documents no longer enumerable anonymously on the durable profile; webhook sender never follows redirects (SSRF admission judged on the registered URL only), bounded timeout, non-2xx ⇒ retry then dead-letter; no `Authorization` header leaves the process.
- **H. Tenancy impact:** enforced and test-proven — ONE shared registry keyed by `tenant_id`; tenant B's subscription receives nothing for tenant A's execution; payload `tenant_id` = owner. No admin-only execution path; the documents gate reuses the existing shared admin admission.
- **I. Persistence impact:** NONE — the subscription map is process-local (same posture as the queue and the pre-R194 `create_app` default). Durable subscriptions NOT claimed.
- **J. Authorities reused:** see C; nothing duplicated.
- **K. Tests:** RED `ImportError` at 63e6e47f (`evidence/r194/red_r194.txt`) → GREEN 15/15 (`green_r194.txt`: 9 headers/documents + 6 delivery/lifespan).
- **L. Regression:** 3087 passed / 14 skipped-xfail / 0 failed across 18 roots at d830ac99 (`regression_d830ac99.txt`).
- **M. Static:** mypy strict clean (3 files); ruff check/format clean; budget guards 33/33.
- **N. NOT claimed:** durable/shared subscription storage; signed webhook payloads; N-5 checkpoints; a CSP without `style-src 'unsafe-inline'` (needs a UI thaw).
- **O. Operator decisions now at the gate (none pre-approved):** **N-5 path** (R194-F1: compose `build_dev_surface` for tenants vs. Core hook in `ws_write` vs. leave deferred); **AD-1** G-TRUST/G-BIND operator act via the admin change lifecycle (R195); **AD-2** durable dev credentials; **AD-3** read-only `/v1/templates` + template composition (R196; P-R191-01 / P-R192-03); **AD-5** v1 topology; P-R192-02; DEC-03/AD-6; AD-7; **D-03 credential rotation** (operator-owned, overdue).
- **P. Next safe continuation point:** `main = 4f494344` + this records-only closure PR. Next session starts from main, reads the R194 pointer and the last ledger row, and does NOT open R195/R196 or any AD-n without an operator ruling recorded here.

**Gates:** gate of record fresh clone `141eb531` PASS **3852/0/0/64** + gateway **194**; D-6 ratchet `min_passed` 3837 → 3852 (+15); PR #46 merged by merge commit → `main 4f494344`; post-merge fresh clone PASS **3852/0/0/64** + gateway **194** (`evidence/r194/gate_merge_4f494344.txt`, `gateway_merge_4f494344.txt`).

### R195-DEC-01 — R195 OPENED (operator "APPROVE R195" with D1–D4 accepted): governed dev-binding registration + remote trust via the EXISTING admin change lifecycle (AD-1) + durable dev credential custody through the EXISTING Vault SecretManagerPort (AD-2); ceiling 4 (2026-09-20)

**Operator rulings recorded (verbatim anchors, "APPROVE THE AUDIT RECOMMENDATIONS"):**
- **AD-1 / G-BIND / G-TRUST:** "use the existing Admin change lifecycle for governed binding registration and remote trust grant/revoke; no new `/v1/dev` write routes."
- **AD-2:** "use the existing durable infrastructure `SecretManagerPort` for dev credentials; no new credential system."
- **AD-3:** "approve read-only `/v1/templates` + `/v1/templates/{ref}` for system templates; record workspace ownership as the later ownership direction; compose the existing built-in template registry." (R196 — NOT opened here.)
- **AD-4:** "keep `SkillManifest` DATA-only semantics for v1."
- **AD-5:** "v1 production topology = durable single-server, single API process, external Postgres; multi-replica future."
- **AD-6 / DEC-03:** "application-scoped credentials deferred beyond user testing."
- **AD-7:** "tenant-code sandbox / App Factory code execution deferred until a sandbox architecture is explicitly approved."
- **D-03:** "credential rotation + history purge required before public deployment; never claim completion without evidence."
- **N-5:** "do NOT choose silently; remains an explicit operator decision unless repository evidence establishes a safe existing composition path."
- "Now proceed strictly by the dependency graph from the audit. Start only with the next valid approved round. Do not pre-approve future implementation details beyond these decisions. Preserve the shared multi-tenant architecture, all existing security, authorization, approval, contract, provider, and Core boundaries. STOP at any new architectural ambiguity or any decision not covered above and return for operator approval. Proposal → approval → implementation → verification → gate."

**Operator text for this round (verbatim):** "APPROVE R195. Accept the proposed R195 engineering decisions: D1: AdminAction +3 members. D2: map all three actions to AdminArea.TOOLS. D3: use the one shared durable Vault SecretManagerPort. D4: exclude N-9 from R195. Execute R195 exactly as proposed. Preserve the shared multi-tenant architecture and all existing security, authorization, approval, contract, provider, and Core boundaries. Do not pre-approve any future round or new architectural decision. Stop at the next decision gate and wait for operator approval."

**Baseline:** `main = 26e24f1e` (R194 closed, PR #47), clean, open PRs 0. Branch `r195_governed_dev_bindings`.

**Repository findings the round rests on (read-only, verified before proposing):**
- F1 — `core.contracts.admin` is NOT among the 15 frozen modules of `contract_freeze_baseline.json` ⇒ widening `AdminAction` is not a frozen-contract change; no route is added ⇒ `served_routes_v1` stays 44; `--check` must MATCH.
- F2 — `GET /v1/admin/capabilities/actions` derives from `ACTION_AREA` (test `len(actions) == len(AdminAction) == len(ACTION_AREA)`) ⇒ the three actions are auto-discovered by the Admin UI; no UI change in this round.
- F3 — `RepoBindingRegistry` (`apps.agent_dev.git_tools`) has `register/get/list_for_tenant` and NO `remove` ⇒ rollback of a registration needs one additive, tenant-scoped method (R195-C).
- F4 — import-linter forbids `core → apps` ⇒ `AdminConfigService` receives the registries through Protocols declared in core (same pattern as `RemoteTrustPort`).
- F5 — `apps/composition/secrets.py::vault_settings_from_env` + `build_secret_manager` exist, are exported, and have ZERO callers in `runtime.py`; runtime always constructs `InMemorySecretManager()` as the `inner` custody. `docs/OPERATIONS.md §2` already claims `VAULT_ADDR` binds a Vault-backed `SecretManagerPort` — that claim is not implemented today. Without Vault a dev `credential_ref` never resolves in a deployed process (`CREDENTIAL_UNRESOLVED`) ⇒ AD-2 is a hard prerequisite of AD-1 being usable.
- F6 — `AdminDraftRequest._refuse_credential_material` (R176 FIX-04) refuses credential-shaped keys at any depth; `credential_ref` is a sanctioned opaque handle ⇒ tokens can never ride an admin change.

**Decision (declared BEFORE any production commit):**
- **R195-A — `core/contracts/admin.py`:** `AdminAction` **+3** (`REGISTER_REPO_BINDING = "register_repo_binding"`, `GRANT_REMOTE_TRUST = "grant_remote_trust"`, `REVOKE_REMOTE_TRUST = "revoke_remote_trust"`; 14 → 17, closed-set widening APPROVED as D1, precedent R177-FIX-03); `ACTION_AREA` maps all three → `AdminArea.TOOLS` (D2; `TOOLS ∈ FINAL_ACTIVE_ADMIN_AREAS` so no second closed-set change).
- **R195-B — `core/admin/service.py`:** Protocols `RepoBindingRegistryPort` (`register`, `get(binding_id, *, tenant_id)`, `remove(binding_id, *, tenant_id)`, `list_for_tenant`) and `RemoteTrustRegistryPort` (`get`, `grant`, `revoke(tenant_id, remote_url, *, revoked_by)`); `AdminConfigService.__init__(..., repo_bindings=None, remote_trust=None)`; `PAYLOAD_FIELD_RULES`: REGISTER_REPO_BINDING `binding: object (contract RepoBinding)`; GRANT_REMOTE_TRUST / REVOKE_REMOTE_TRUST `target_tenant_id: uuid`, `remote_url: string`, `note: string optional` (grant only). Validation: absent seam ⇒ "repo bindings registry seam is not bound in this composition" / "remote trust registry seam is not bound in this composition"; `RepoBinding.model_validate` errors named; duplicate binding id refused; `remote_url` must be `https://` (contract `RemoteUrl` reused via `RemoteTrustGrant.model_validate`); REVOKE refuses when no effective grant exists ("nothing to revoke" — the runtime `revoke` would otherwise record a phantom row). Preview: plain-language impact. Snapshot: `{registered: False}` for binding; `{trust_grant: prior | None}` for grant/revoke. Apply: `repo_bindings.register(binding)`; `remote_trust.grant(RemoteTrustGrant(tenant_id=target, remote_url, trusted=True, granted_by=str(actor_id), granted_at=now, note))`; `remote_trust.revoke(target, remote_url, revoked_by=str(actor_id))`. Restore: binding → `remove(id, tenant_id=binding.tenant_id)`; grant → prior grant re-`grant`ed if one existed else `revoke(..., revoked_by=str(actor_id))` (auditable, never silent deletion); revoke → prior effective grant re-`grant`ed (the rollback actor becomes `granted_by`, note preserved) else `RollbackUnavailable`.
- **R195-C — `apps/agent_dev/git_tools.py`:** `RepoBindingRegistry.remove(binding_id, *, tenant_id) -> RepoBinding` — unknown ⇒ `BINDING_UNKNOWN`, foreign ⇒ `BINDING_TENANT_MISMATCH` (identical to `get`), then delete + re-save.
- **R195-D — `apps/composition/runtime.py`:** `vault = vault_settings_from_env(env_dict)`; `custody: SecretManagerPort = build_secret_manager(vault) if vault is not None else InMemorySecretManager()` handed to `onboarding_secrets_from_env(..., inner=custody)` (the ONE shared custody instance — D3; ADR-0007 "swapped HERE only"; half-configured Vault raises inside `vault_settings_from_env`, no silent fallback); `AdminConfigService(..., repo_bindings=dev.bindings if dev is not None else None, remote_trust=dev.trust if dev is not None else None)`.
- **Ceiling 4** — a fifth production file ⇒ STOP. **N-9 excluded** (D4).

**Invariants preserved (to be proven by tests):** shared multi-tenant — binding payload carries explicit `tenant_id`; trust keyed `(tenant_id, remote_url)`; `RepoBindingRegistry.get/remove` refuse foreign tenants with `BINDING_TENANT_MISMATCH`; `_require_trust` stays fail-closed. No new route; no new area; no admin-only execution path; approval lifecycle, `AdminPersistencePort`, `AuditLogPort` rows (`ADMIN_CONFIG_PUBLISHED/ROLLED_BACK`) reused unchanged; contract SHAPE unchanged (`--check` MATCHES); secrets never in payloads/logs/repr; in-memory profile byte-identical without `VAULT_ADDR`.

**RED-first proof plan:** `tests/admin/test_r195_governed_dev_bindings.py` — actions discovered on `/v1/admin/capabilities/actions` with area `tools`; draft→validate→publish registers a binding visible to `get(..., tenant_id=owner)` and refused for a foreign tenant; duplicate id refused at validate; absent seams ⇒ named validation reasons; rollback removes the binding (store re-saved, load has no resurrection); grant ⇒ `is_trusted` True; revoke ⇒ False with `revoked_by = actor`; revoke without grant refused at validate; rollback of grant (no prior ⇒ revoke row) and of revoke (prior ⇒ re-granted; none ⇒ `RollbackUnavailable`); payload carrying `token` still refused by `AdminDraftRequest`; previews non-empty; `remove` tenant scoping. `tests/composition/test_r195_secret_custody.py` — `VAULT_ADDR`+`VAULT_TOKEN` ⇒ `VaultSecretManager` bound (hvac double from `tests/infrastructure/test_vault_secret_manager_t074.py`; `hvac.Client` patched at `apps.composition.secrets.hvac`); `VAULT_ADDR` alone ⇒ `ValueError`; absent ⇒ `InMemorySecretManager`; profile with `AGENT_DEV_STATE_DIR` exposes `AdminConfigService` seams bound (published binding visible in `dev.bindings`).

**Exit:** GREEN; regression over the 18 roots; freeze `--check` MATCHES; mypy strict + ruff on touched files; budget guards 33/33; fresh-clone gate + gateway 194; D-6 ratchet at the gate of record; PR by merge commit; post-merge gate; R195-DEC-02; records (`OPERATIONS §2` Vault rows now TRUE + dev-credential seeding procedure; `CAPABILITY_MAP`; `BACKEND_STATE_OF_TRUTH §E` G-BIND/G-TRUST/G-CRED; `ADMIN_UI_BACKEND_COVERAGE §5`); then STOP at the operator decision gate (R196/AD-3 and later NOT pre-approved; D-03 NOT claimed).

### R195-DEC-02 — R195 CLOSED: governed dev-binding registration + remote trust grant/revoke through the existing admin change lifecycle (AD-1) and the shared Vault `SecretManagerPort` bound at the composition root (AD-2); 4 production files, contract shape unchanged; gate of record 3872/0/0/64; PR #48 merged (2026-09-20)

**Operator text (verbatim anchors):** "APPROVE R195. Accept the proposed R195 engineering decisions: D1: AdminAction +3 members. D2: map all three actions to AdminArea.TOOLS. D3: use the one shared durable Vault SecretManagerPort. D4: exclude N-9 from R195. Execute R195 exactly as proposed. … Do not pre-approve any future round or new architectural decision. Stop at the next decision gate and wait for operator approval."

**Final verification (A–P):**
- **A. Verified TRUE (F1–F6 of R195-DEC-01):** `core.contracts.admin` not frozen; actions auto-discovered; `RepoBindingRegistry` lacked `remove`; core→apps forbidden; Vault composition existed with zero callers (the `OPERATIONS §2` claim was untrue until R195-D); credential-material guard already refuses tokens in payloads.
- **B. Disproved / withdrawn:** nothing in scope. One test-side correction during GREEN: `GitRefusalCode` lives in `core.contracts.repo_binding` (no `core.tools.git_refusal` module) — test import fixed, production untouched.
- **C. Already implemented (reused, not rebuilt):** 21 §3 lifecycle (`draft/validate/preview/publish/rollback`), `PAYLOAD_FIELD_RULES` + `field_rule_problem`, `ACTION_AREA` discovery read model, `AuditLogPort` rows, `RepoBindingRegistry`, `RemoteTrustRegistry` (`grant`/`revoke` record the actor), `JsonBindingStore`/`JsonRemoteTrustStore`, `vault_settings_from_env` + `build_secret_manager` + `VaultSecretManager`, `onboarding_secrets_from_env(inner=)`, `build_dev_bindings`.
- **D. Actually changed (4 files = ceiling 4):** `core/contracts/admin.py` (+13) — `AdminAction` 14→17, `ACTION_AREA` +3 → `TOOLS`; `core/admin/service.py` (+187/−1) — `RepoBindingRegistryPort`/`RemoteTrustRegistryPort`, `AdminConfigService(repo_bindings=, remote_trust=)`, rules, validation, preview, snapshot, apply, restore; `apps/agent_dev/git_tools.py` (+13) — `RepoBindingRegistry.remove(binding_id, *, tenant_id)`; `apps/composition/runtime.py` (+30/−1) — `secret_custody_from_env`, seams handed to the admin service.
- **E. Why each file was necessary:** the closed set lives in `core/contracts/admin.py`; the lifecycle lives in `core/admin/service.py`; rollback needs a tenant-scoped removal the registry lacked; `runtime.py` is the only production composition root (ADR-0007 "swapped HERE only").
- **F. Contract shape impact:** NONE — `--check` MATCHES at the gate of record `2a636b50` and post-merge `16048078`; `served_routes_v1` 44 unchanged (`evidence/r195/CONTRACT_FREEZE_RECORD_R195_UPDATE.md`).
- **G. Security impact:** positive — binding registration and trust grant/revoke are now governed (validated, previewed, audited `ADMIN_CONFIG_PUBLISHED/ROLLED_BACK`, reversible) instead of absent/hand-edited JSON; secrets never in payloads (R176 FIX-04 re-proven); Vault custody is all-or-nothing (half-configured raises at startup); revoke never records a phantom row; rollback of a grant is an explicit auditable revocation, never a silent deletion.
- **H. Tenancy impact:** enforced and test-proven — binding visible to its tenant only (`BINDING_TENANT_MISMATCH` for a foreign tenant, also on `remove`); trust keyed `(tenant_id, remote_url)` (tenant B untouched by tenant A's grant); Vault paths tenant-prefixed (`exists` false for a foreign tenant). No admin-only execution path; the shared admin admission is reused.
- **I. Persistence impact:** bindings/trust persist through the existing JSON stores (re-saved on register/remove/grant/revoke; a fresh registry over the same store sees the published state and never resurrects a rolled-back binding). Vault persists credentials when configured. `AdminPersistencePort` (entity catalogs) untouched — dev bindings are not catalog entities.
- **J. Authorities reused:** see C; nothing duplicated; no new area, no new route, no new credential system.
- **K. Tests:** RED collection errors at c9b7f754 (`evidence/r195/red_r195.txt`) → GREEN 20/20 (`green_r195.txt`: 16 lifecycle/tenancy/rollback + 4 custody/composition). Pinned suites (tests/admin, R179 discovery, R193 composition, tests/agent_dev) 54/54.
- **L. Regression:** 18 roots at fd31a8cd — 3107 passed / 14 skipped-xfail / 0 failed (`regression_fd31a8cd.txt`).
- **M. Static:** mypy strict clean (4 files); ruff format/check clean; budget guards 33/33; import-linter (in gate) PASS.
- **N. NOT claimed:** D-03 rotation + history purge evidence (operator-owned, still required before public deployment); N-9 branch/PR cleanup primitive (D4 excluded); Admin UI forms for the three actions (R197); AD-3 templates (R196); a live Vault round-trip (adapter proven against the hvac KV v2 double; live smoke remains `tests/infrastructure/test_vault_live_smoke.py`, credential-gated).
- **O. Operator decisions now at the gate (none pre-approved):** **R196** — AD-3 read-only `/v1/templates` + `/v1/templates/{ref}` (additive route ⇒ baseline re-derive + freeze record in-round; workspace ownership recorded as later direction); then R197 (UI closure incl. the three new action forms), R198 (final acceptance: AD-5 topology artefact, D-03 evidence). N-5 checkpoints remains an explicit operator decision (no safe existing composition path found).
...
### R196-DEC-02 — R196 CLOSED: read-only `/v1/templates` + `/v1/templates/{ref}` served for system templates and the built-in template registry composed (AD-3); DECLARED additive shape change executed (44 → 46 served routes; frozen modules identical); 4 production files; gate of record 3888/0/0/64; PR #50 merged (2026-09-20)

**Operator text (verbatim anchors):** "APPROVE R196 with confirmation" (D1–D5 as proposed).

**Final verification (A–P):**
- **A. Verified TRUE (F1–F10 of R196-DEC-01):** `TemplateRegistry` existed with zero production callers; exactly one built-in system template (`APP_FACTORY_TEMPLATE`); `StrategyExecutor` template mode resolved nothing in production; `core.contracts.agent_template` not frozen; `served_routes_v1` derived from a hermetic `create_app` that had to bind the seam; `CAPABILITY_IDS` closed at 23; unknown-resource convention 404 `validation_error`; `TemplateOrigin` is provenance, not ownership.
- **B. Disproved / withdrawn:** nothing in scope. One test-side pin moved as declared (`len(CAPABILITY_IDS)` 23 → 24) and the R179 shelf family map gained `/v1/templates → templates.listing` (its own guard demanded it: "every mounted family has a shelf row").
- **C. Already implemented (reused, not rebuilt):** `TemplateRegistry.list/resolve_ref/as_strategy_mapping`, `StrategyTemplate` (served whole as the detail body), `StrategyExecutor(templates=)`, `_principal`, `error_response(... http_status=404)`, `Capability` catalog + `_cap`, `contract_freeze_derive.py`, `APP_FACTORY_TEMPLATE`.
- **D. Actually changed (4 files = ceiling 4):** `core/contracts/agent_template.py` (+52) — `TemplateListEntry.from_template`, `TemplatesListResponse`; `apps/api/app.py` (+67/−3) — `templates=` seam, one-source guard, executor wiring, two GET routes, capability row, `app.state.strategy_executor`; `apps/composition/runtime.py` (+26/−1) — `build_template_registry`, `RuntimeProfile.templates`; `apps/api/capabilities.py` (+3) — `templates.listing`. Verification tooling: `contract_freeze_derive.py` binds the seam; `contract_freeze_baseline.json` re-derived.
- **E. Why each file was necessary:** the read model belongs with the template contract; the FastAPI object and principal resolver live in `app.py`; `runtime.py` is the only composition root; the catalog id set is closed in `capabilities.py`.
- **F. Contract shape impact:** ADDITIVE and DECLARED — `served_routes_v1` 44 → 46 (`+GET /v1/templates`, `+GET /v1/templates/{ref}`), 0 removals; `frozen_modules` (15), `modules`, `resource_signal_snapshot` byte-identical (verified programmatically, freeze record §1); `--check` MATCHES at the gate of record and post-merge. `ExecutionStrategySpec.template_id` (frozen) consumed, unchanged.
- **G. Security impact:** neutral-to-positive — two authenticated read routes over platform data; unknown/inactive/non-system refs collapse into one 404 (anti-enumeration); no write path; stages cannot name providers (contract-enforced); unified error envelope; hardening headers apply (outermost middleware, R194).
- **H. Tenancy impact:** system templates are tenant-neutral platform data behind tenant authentication (test-proven 401 tokenless / 200 user); no tenant-owned template exists or is accepted; template-mode executions stay tenant-scoped via the existing execution path.
- **I. Persistence impact:** NONE — registry is process-local, seeded from code (`APP_FACTORY_TEMPLATE`). Durability NOT claimed (P-R192-03 open).
- **J. Authorities reused:** see C; no new registry, no second executor, no new error code, no new area.
- **K. Tests:** RED bd94ad04 (`evidence/r196/red_r196.txt`) → GREEN 16/16 (`green_r196.txt`: 3 contract + 10 routes + 3 composition). Guards + catalog/shelf suites 58/58.
- **L. Regression:** 18 roots at 8f084086 — 3123 passed / 14 skipped-xfail / 0 failed (`regression_8f084086.txt`).
- **M. Static:** mypy strict clean (4 files); ruff format/check clean; budget guards 33/33.
- **N. NOT claimed:** template durability; user/workspace templates or any write route (P-R191-01 write half, P-R192-03); App Factory code generation / sandbox (AD-7); the UI template picker (R197); D-03 rotation/purge evidence; live-model execution of `app_factory.plan` (resolution proven hermetically; execution runs through the ONE executor with routed child executions).
- **O. Operator decisions now at the gate (none pre-approved):** **R197** — UI closure (template picker over `/v1/templates`; forms for `register_repo_binding` / `grant_remote_trust` / `revoke_remote_trust` over the existing action discovery read model; frozen UI trees require a declared thaw); **R198** — final acceptance (AD-5 topology artefact; D-03 evidence; final acceptance definition from the closure audit). N-5 checkpoints remains an explicit operator decision. P-R192-03 (workspace ownership) remains a recorded direction only.
- **P. Next safe continuation point:** `main = e08b04f6` + this records-only closure PR. Next session starts from main, reads `R196_POINTER` and ledger row 9, and does NOT open R197/R198 without an operator ruling recorded here.

**Gates:** gate of record fresh clone `63f7b876` PASS **3888/0/0/64** + gateway **194**; D-6 ratchet `min_passed` 3872 → 3888 (+16); PR #50 merged by merge commit → `main e08b04f6`; post-merge fresh clone PASS **3888/0/0/64** + gateway **194** (`evidence/r196/gate_merge_e08b04f6.txt`, `gateway_merge_e08b04f6.txt`).

### R197-DEC-01 — R197 opened: UI closure — Workbench template picker over `/v1/templates`; R195 admin actions closed by discovery evidence; `ui/app/` thawed and newly guarded; ceiling 0 (2026-09-21)
- **Operator text (verbatim):** "produce the R197 proposal (read-only seam verification first) and stop for approval" → proposal delivered → "APPROVE R197 (optionally with rulings on D1–D5)". No rulings overriding the proposal were given; the recommended rulings therefore stand: **D1** thaw `ui/app/` (Workbench) ONLY — `ui/admin` (73 = N0, zero headroom) and `ui/app/command` (12, zero headroom) stay frozen; **D2** declare `ui_workbench_static_check` BEFORE the first Workbench edit, `/v1/` ceiling `null` → measured at first GREEN, moves down only; **D3** the R180 Q5 discovery-driven change form is the closure for `register_repo_binding` / `grant_remote_trust` / `revoke_remote_trust` — no bespoke forms, no `ui/admin` edit; **D4** picker options = server-order `templates[].ref`, sent as `execution_strategy {mode:"template", template_id: ref}`, nothing sent when no template is chosen, `/v1/templates/{ref}` NOT consumed; **D5** `round_r197` ceiling **0**.
- **Findings (read-only, measured at `main b8f2f706`):** `ui/admin/app.js` `/v1/` = 73 = N0 (65 code + 8 comment-only); `ui/app/command/command.js` = 12 = ceiling (all served literals); `ui/app/app.js` = 21 and UNGUARDED statically (only `tests/composition/test_ui_app_pd2.py` behavioural pins); `ui/app/app.js` carries **4 `fetch(`** calls (`api()`, DELETE workspace, DELETE project, SSE reader) — so the inherited "exactly one fetch( inside api()" rule is NOT claimed for this tree; the block declares `fetch_count_ceiling_app_js: 4` as measured (moves down only; R197 adds none). `AdminArea.TOOLS ∈ FINAL_ACTIVE_ADMIN_AREAS`, so the console's `populateActionSelect()` already offers the three R195 actions with `PAYLOAD_FIELD_RULES` hints — no UI change is needed for R197-B. `_StrategyView.__getitem__` → `resolve_ref(key)`, so the listed `ref` is exactly what template-mode execution accepts (R196 D5).
- **Decision:** open R197 by declaration. Files allowed to change: `ui/app/app.js`, `ui/app/index.html`, `ui/app/styles.css` (only if the select needs a rule); NEW `tests/ui/test_workbench_template_picker_r197.py`, `tests/ui/test_r197_admin_actions_discovered.py`; manifest (`ui_workbench_static_check`, `round_r197`); records. FROZEN: `core/ apps/ infrastructure/` (ceiling 0, comments included), `ui/admin/*`, `ui/app/command/*`, all contracts, freeze baseline (46 routes; `--check` must MATCH unchanged).
- **Invariants:** the Workbench's ONLY template source is `GET /v1/templates` (no hardcoded ids/refs); the read runs after the session is established, never at module load; a refused read renders the unified error and offers no option; `execution_strategy` is sent only when a ref is chosen; `ui/admin/app.js` stays byte-identical (73); `command.js` stays 12; `/v1/templates/{ref}` not consumed; no new `fetch(`; no new dependency; ADR-0013 vanilla ES module posture.
- **RED plan:** `tests/ui/test_workbench_template_picker_r197.py` (manifest block equality + ceilings; `ask-template` select present; `/v1/templates` literal present and served; `execution_strategy`/`template_id` wiring present; populate call not at module load; no hardcoded template id; transports; frozen trees unchanged) fails at b8f2f706 because the select/literal are absent and the ceiling is `null` (fails closed). `tests/ui/test_r197_admin_actions_discovered.py` (served-app: the three actions appear in `GET /v1/admin/capabilities/actions` with their fields; `ui/admin/app.js` has no hand-list and count 73) is expected GREEN already — it is evidence, recorded as such, not claimed as RED.
- **Exit:** GREEN on both modules + `tests/ui` + `tests/composition/test_ui_app_pd2.py` + 18 regression roots; live Chromium DOM probe of the picker (`evidence/r197/`); freeze `--check` MATCHES; fresh-clone gate PASS + gateway 194; PR by merge commit; post-merge gate; records-only closure PR; STOP.

### R197-DEC-02 — R197 CLOSED: Workbench template picker live over `/v1/templates`; R195 admin actions closed by discovery evidence; `ui/app/` guarded (ceiling 22); production ceiling 0/0 (2026-09-21)
- **A. Operator text (verbatim):** "APPROVE R197 (optionally with rulings on D1–D5)" — no override; recommended rulings D1–D5 stand (R197-DEC-01).
- **B. Declaration before code:** `addec78d` — `ui_workbench_static_check` (files exact; `v1_count_ceiling_app_js` **null**, fails closed; `fetch_count_ceiling_app_js` **4** measured pre-round; exception 0; banned transports; forbidden literals) + `round_r197` ceiling **0**.
- **C. RED:** `aec5efe2` — `tests/ui/test_workbench_template_picker_r197.py` 6 FAILED as planned (`evidence/r197/red.txt`); `tests/ui/test_r197_admin_actions_discovered.py` GREEN from the start (evidence, declared as such).
- **D. Implementation (UI only, outside counted roots):** `ui/app/index.html` +4 (`<select id="ask-template">`, only the empty default option in HTML); `ui/app/app.js` +27 (`populateTemplateSelect()` from `enterMain`, options = server rows keyed by `ref`, stale options cleared before the result is inspected, refusal renders `renderError` verbatim; `submitAsk` sets `execution_strategy {mode:"template", template_id: ref}` ONLY when a ref is chosen). `/v1/` 21 → **22**; `fetch(` 4 → 4.
- **E. Ceiling set ONCE:** `v1_count_ceiling_app_js` null → **22** at the first GREEN (`27d37dec`); rule text records history; moves down only.
- **F. Frozen trees held:** `git diff b8f2f706..8ced2242 -- core apps infrastructure ui/admin ui/app/command` EMPTY; `ui/admin/app.js` 73 = N0; `command.js` 12; pinned by `test_frozen_ui_trees_unchanged`.
- **G. Contract freeze:** no shape change; `--check` MATCHES (46 routes) — `evidence/r197/CONTRACT_FREEZE_RECORD_R197_UPDATE.md`.
- **H. Live proof:** real `python -m apps.main` (demo principal) + real Chromium — `evidence/r197/workbench_template_picker_dom_probe.json`: `/v1/templates` read AFTER `/v1/auth/session`; options `["", "app_factory.plan@1"]`; POST `/v1/execute` WITH template → body carries `execution_strategy.template_id="app_factory.plan@1"` → 200 `succeeded`; WITHOUT template → body `{"ask": …}` unchanged → 200. (First probe draft mis-captured request bodies via the response hook — replaced with request-time capture; the recorded run is the corrected one.)
- **I. Regression:** 18 roots at `27d37dec` 3123 passed / 14 skipped-xfail / 0 failed (= R196).
- **J. Gate of record:** first attempt `41ba57a8` **FAIL** — `ruff check .` over the probe script stored as `evidence/r197/*.py` (renamed `.py.txt`; ledger row 5, kept). Re-gate fresh clone **`cf00399a` PASS 3914/0/0/64**, `round_r197=0/0`, gateway **194**. D-6 ratchet `min_passed` 3888 → **3914** (+26 = 22 + 4).
- **K. Merge:** PR #52 by merge commit → **`main 8ced2242`** (preconditions: mergeable clean, protection 404, 0 statuses/check-runs/workflows).
- **L. Post-merge gate:** fresh clone `8ced2242` **PASS 3914/0/0/64**, gateway 194 (`evidence/r197/gate_merge_8ced2242.txt`). `pytest.last_measured` → 8ced2242.
- **M. Closed:** UI half of AD-3 (template selection reachable from the Workbench); R195 UI closure by D3 (discovery form; no bespoke forms — a hand-list would re-introduce what R180 abolished). The Workbench is now statically guarded for the first time.
- **N. NOT claimed:** template detail view (`/v1/templates/{ref}` not consumed); user/workspace templates; write routes; App Factory generation (AD-7); Command Center picker; D-03 rotation/purge evidence; any `core/ apps/ infrastructure/` change.
- **O. Operator decisions now at the gate (none pre-approved):** **R198** — final acceptance: AD-5 topology artefact (durable single-server, single API process, external Postgres), D-03 credential rotation + history purge evidence (operator-owned; never claimed without evidence), N-9 branch cleanup. Any further UI work (template detail, Command Center picker) = its own proposal.
- **P. Next safe continuation point:** `main = 8ced2242` + this records-only closure PR. Next session starts from main, reads `R197_POINTER` and ledger row 9, and does NOT open R198 without an operator "APPROVE R198".

**Gates:** gate of record fresh clone `cf00399a` PASS **3914/0/0/64** + gateway **194**; D-6 ratchet `min_passed` 3888 → 3914 (+26); PR #52 merged by merge commit → `main 8ced2242`; post-merge fresh clone `8ced2242` PASS 3914/0/0/64 + gateway 194.

### R198-DEC-01 — R198 opened: final acceptance "as measured" — ADR-0014 (AD-5), live durable re-measure, V1 acceptance register, D-03 evidence slot; ceiling 0 (2026-09-21)
- **Operator text (verbatim):** "APPROVE R198. Confirm the following D1–D6 rulings: D1 — YES: Accept v1 "as measured"; D-03 remains explicitly OPEN until operator evidence is supplied. Do not claim Production Ready while D-03 is unresolved. D2 — YES: Use ADR-0014 for the AD-5 topology artefact. D3 — YES: Run the live PostgreSQL 17 + pgvector durable-profile re-measure on current main. If the environment cannot support it, record NOT EVALUATED honestly. D4 — YES: Use the history-purge path: revoke/rotate the exposed credentials and purge the leaked history, including affected legacy branches, with explicit acknowledgment that rewritten SHAs invalidate prior commit references. Do not perform the rewrite unless the required operator-controlled procedure/evidence is available. D5 — Use the disposition required by D4: remove the legacy branches that carry the leaked blobs after preserving any required historical record through the approved purge procedure; do not leave publicly reachable refs to the leaked commits. D6 — YES: Record the UI-unconsumed routes as DECLARED-UNCONSUMED in the acceptance register. Do not create another UI round solely to consume them. Execute R198 exactly within these rulings. Preserve all existing shared multi-tenant, security, authorization, approval, contract, provider, and Core boundaries. Do not silently expand scope or convert deferred items into implementation work. Do not claim any item as closed without evidence. STOP at any new architectural ambiguity or unsupported operator action and return for approval."
- **Findings (read-only audit at `main fbdd1f0b`, R198 proposal):** no AD-5 artefact exists (OPERATIONS §1 holds the facts; no Dockerfile/compose/workflow anywhere; TLS assumed at an undocumented ingress; `engine.py` is `postgresql+asyncpg` only). D-03: tree clean; history carries the R177 canonical-prompt leak (`521d8850` added `GITHUB_TOKEN=ghp_RD…`, `GROQ_API_KEY=gsk_LM…`, `GW_ASSEMBLYAI_API_KEY=512fc6…`; `28989b04` redacted the tree only); GitHub secret-scanning alert #1 (Groq) **OPEN, publicly_leaked**, alert #2 (PAT) dismissed `used_in_tests` — not a rotation proof; repository **PUBLIC**; the five legacy branches (all diverged: +1/−1061, +4/−627, +8..16/−461) carry the blobs. Last real-PostgreSQL durability evidence is R181 (`7a41caff`); 32 production commits since; migrations head unchanged (0022). UI consumes 41 of 46 served `/v1` paths (9 via `${step}` interpolation); truly unconsumed: `GET /v1/admin/evaluations/{id}`, `GET /v1/admin/learning/dashboard` (INERT), `GET /v1/templates/{ref}` (R197 D4), `GET/DELETE /v1/webhooks/{id}`. Security/tenancy/contracts/regression all green (3914/0/0/64; 46 routes MATCH; 29 isolation tests).
- **Decision:** open R198 by declaration as a **records + evidence round, ceiling 0**. Allowed: `docs/architecture/ADR-0014_V1_PRODUCTION_TOPOLOGY.md`, `docs/ai_orchestration_pack/V1_ACCEPTANCE_REGISTER.md`, `evidence/r198/*`, `tests/verification/test_ad5_topology_artefact_r198.py`, `tests/verification/test_v1_acceptance_register_r198.py`, manifest (`round_r198`; `not_evaluated` only if R198-B cannot run), OPERATIONS §1 cross-link (doc only), records. FROZEN: `core/ apps/ infrastructure/`; `ui/*` (73 / 12 / 22); contracts + freeze baseline (46; `--check` MATCHES).
- **Invariants:** "Production Ready" is NOT claimed anywhere while the D-03 register row is OPERATOR-OWNED-OPEN (pinned by test). No history rewrite by engineering without the operator-controlled procedure/evidence (D4); if performed later, rewritten SHAs invalidate every sha cited in ledgers/pointers — acknowledged. Legacy branches are removed ONLY within that procedure (D5). The live re-measure claims exactly what `tests_live/r179` + `tests_live/r178` measure on a disposable local cluster — "single node; no production claim" as the harness itself prints. Deferred items are listed, never implemented.
- **RED plan:** both new `tests/verification` guards fail at fbdd1f0b (documents absent). GREEN after R198-A/C/D. R198-B produces `evidence/r198/durability_measured_<sha>.json` (+ logs) or a NOT EVALUATED declaration.
- **Exit:** GREEN + freeze MATCHES + fresh-clone gate PASS + gateway 194 → PR (merge commit) → post-merge gate → records-only closure → STOP. D-03 and N-9 rows remain OPERATOR-OWNED-OPEN at exit unless evidence arrives in-round.

### R198-DEC-02 — R198 CLOSED: v1 accepted AS MEASURED; ADR-0014 (AD-5) + V1 acceptance register merged; live durable re-measure on current main; D-03 / N-9 OPERATOR-OWNED-OPEN; Production Ready NOT claimed (2026-09-21)
- **A. Operator text (verbatim):** "APPROVE R198. Confirm the following D1–D6 rulings …" (full text in R198-DEC-01). Executed exactly within D1–D6.
- **B. Declaration before code:** `85127178` — `round_r198` ceiling **0**; scope = records + evidence + two static guards under `tests/verification/`.
- **C. RED:** `63fed06d` — 15/15 failing (7 ADR-0014 guards, 8 register guards; documents absent), `evidence/r198/red.txt`.
- **D. R198-A (D2):** `docs/architecture/ADR-0014_V1_PRODUCTION_TOPOLOGY.md` — ruling verbatim; OPERATIONS §1 order (Postgres up → `alembic upgrade head` → `apps.cli serve`); ONE process (API + outbox relay + exec worker + webhook worker, `apps/main.py` lifespan); external PostgreSQL via `DATABASE_URL` (`postgresql+asyncpg`); TLS at an external ingress, `HSTS` only then; exclusions with decision refs (multi-replica ⇐ P-R188-04 + in-process tickets/grants; no distributed worker — no Redis env key read by the runtime; no token streaming; async strategy P-R188-03 NO; no container/CI artefacts). Every env key it names is read by the code (pinned).
- **E. R198-B (D3):** live re-measure on `332db1d3` with PostgreSQL 17.11 (Debian 17.11-0+deb13u1) + pgvector 0.8.0 rebuilt into `.venv/pg_root`; `tests_live/r179` **5/5** (fresh cluster, real alembic 0001→0022, `python -m apps.main` SIGKILL -9 between phases: P1 knowledge/GOLD 1→1, P2 conversation 3→3 + execute 200, P3 audit 2→2, P4 usage 6.0→8.0); `tests_live/r178` **59/59**. Identical to R181 (`7a41caff`). Scope exactly as the harness prints: "disposable local cluster; real alembic; single node; no production claim". `evidence/r198/live_*_332db1d3.*`.
- **F. R198-C (D1/D6):** `docs/ai_orchestration_pack/V1_ACCEPTANCE_REGISTER.md` — sections A (AD-1..AD-7, D-03, N-5, N-9), B (R189 §6.1–6.10, P-R191-01, P-R192-03, not_evaluated #1), C (DECLARED-UNCONSUMED: `GET /v1/admin/evaluations/{id}`, `GET /v1/admin/learning/dashboard`, `GET /v1/templates/{ref}`, `GET/DELETE /v1/webhooks/{id}`), D (measured cross-cutting facts). Closed status set; guard forbids a "Production Ready" claim while D-03 is OPERATOR-OWNED-OPEN and binds N-9 to D-03.
- **G. R198-D (D4/D5):** `evidence/r198/D03_CREDENTIAL_ROTATION_AND_PURGE.md` — measured state (leak `521d8850`; tree-only redaction `28989b04`; alert #1 Groq OPEN publicly_leaked; alert #2 PAT dismissed `used_in_tests`, NOT a rotation proof; repo PUBLIC; legacy branches carry the blobs; session token ≠ leaked token is NOT revocation evidence) + operator checklist E1–E9 (revocations, pre-purge bundle, `git filter-repo` over `main` + legacy branches, branch/tag removal, GitHub GC, alert closure, SHA-invalidation acknowledgment). **Engineering performed no rewrite, force-push, branch deletion or alert dismissal.**
- **H. Frozen trees held:** `git diff fbdd1f0b..dc26e3e6 -- core apps infrastructure ui` EMPTY; freeze `--check` MATCHES (46).
- **I. Regression:** 18 roots at `8805e024` 3138 / 14 / 0 (+15 static guards vs R197).
- **J. Gate of record:** fresh clone **`8805e024` PASS 3929/0/0/64**, `round_r198=0/0`, gateway 194. D-6 ratchet `min_passed` 3914 → **3929**.
- **K. Merge:** PR #54 by merge commit → **`main dc26e3e6`** (mergeable clean; protection 404; 0 statuses/check-runs).
- **L. Post-merge gate:** fresh clone `dc26e3e6` **PASS 3929/0/0/64**, gateway 194. `pytest.last_measured` → dc26e3e6.
- **M. Honesty notes:** a sandbox reset wiped an unpushed first draft of ADR-0014 / register / D-03 slot and the first live run — all redone and re-measured (ledger row 3 note). The r179/r178 harness rewrites `evidence/r179/*.json` in place; those files were restored (`git checkout`) and the fresh outputs stored under `evidence/r198/` with the measuring sha.
- **N. NOT claimed:** **Production Ready**; D-03 closure; N-9 closure; multi-replica readiness; container/ingress/CI manifests; live two-account provider failover; any R189 §6 deferred item; any `core/ apps/ infrastructure/ ui/` change.
- **O. Operator actions now owed (no engineering pre-approval):** complete E1–E9. When the artefacts exist: a records-only **R199** verifies them by API where possible, flips the D-03 / N-9 rows to ACCEPTED-AS-MEASURED, and re-baselines pointers on the rewritten history (bundle = SHA map).
- **P. Next safe continuation point:** `main = dc26e3e6` + this records-only closure PR. Next session starts from main, reads `R198_POINTER` and ledger row 9, and does NOT start any round without an operator instruction.

**Gates:** gate of record fresh clone `8805e024` PASS **3929/0/0/64** + gateway **194**; D-6 ratchet 3914 → 3929 (+15); PR #54 merged by merge commit → `main dc26e3e6`; post-merge fresh clone `dc26e3e6` PASS 3929/0/0/64 + gateway 194.

### R199-DEC-01 — R199 opened: Unified Shell Foundation — one brand per surface, shared tokens in Command, cross-navigation, /app provider truth; UI-only, ceiling 0 (2026-09-21)
- **A. Operator text (verbatim):** "APPROVE R199. Accept D1–D5 as proposed: - D1 = A - D2 = A - D3 = QEVION · Command / QEVION · Workbench / QEVION · Admin - D4 = use only real catalog/API provider-state values - D5 = standard gate ratchet. Execute R199 exactly as proposed. Do not pre-approve R200 or R204. Preserve the unified shared multi-tenant product direction and all existing security, authorization, Core, contract, and provider boundaries. Stop at the next decision gate."
- **B. Findings (read-only product audit at `main f1a9752a`, PARTs 1–15):** three static trees, three logins, zero cross-links, `GET /` 404; three brand strings; `command.css` carries a divergent token set (`--brand #7dd3fc` / `#38bdf8`) while `ui/app/styles.css` and `ui/admin/styles.css` share an identical `:root`; `GET /v1/models` serves `providers[]` (R188 C4) and `/app` does not render it — an authenticated in-memory session sees `local-echo-1 · available` with the "no real model was called" fact only inside result text; Command is admin-only by UI gate (`probeSession`, `is_admin !== true` ⇒ token cleared) and its execute path sends `{ask}` only. No MOCK data and no fabricated state found in any tree.
- **C. Rulings applied:** D1 = A — Command keeps its admin-only posture this round; links only, no gate change. D2 = A — root `/` stays 404; `/app/command/` is the documented entry. D3 — brand strings exactly `QEVION · Command`, `QEVION · Workbench`, `QEVION · Admin`. D4 — the provider-truth strip and provider chips quote only values served by `GET /v1/models` (provider keys, availability, counts); no prose invented client-side; no provider-name branching. D5 — D-6 ratchet at the gate of record only.
- **D. Decision:** open R199 by declaration as a **UI-only round (layer A), ceiling 0**. Allowed: `ui/app/command/{index.html,command.css}`, `ui/app/{index.html,app.js,styles.css}`, `ui/admin/{index.html,styles.css}`, `tests/ui/test_unified_shell_r199.py` (RED first), the three test pins on the old Command title (`tests/ui/test_command_center_topology_r182.py`, `tests/ui/test_command_center_browser_r182.py`, `tests/ui/test_command_center_browser_r185.py`) and `tests/composition/test_admin_console_runtime.py:80` if its `Admin Console` substring pin fails, `evidence/r199/*`, records (this log, `R199_HANDOFF.md`, `PROJECT_EXECUTION_STATE.md`, manifest `round_r199`, `evidence/r199_state_ledger.md`). FROZEN: `core/ apps/ infrastructure/` (0 files); `ui/app/command/command.js` (12, by identity); `ui/admin/app.js` (73 = N0, by identity); contracts + freeze baseline (46; `--check` must MATCH). `ui_workbench_static_check` stays 22 / 4 (no new `/v1/` literal, no new `fetch(`).
- **E. Invariants:** one execution path (`POST /v1/execute`), one Agent authority (`/v1/agent/*`, admin), one Router — untouched. No new routes, no new transport. Every rendered provider/availability word originates in the served response. No `href` carries `/v1/`. Command's motion collapse under `prefers-reduced-motion` (R185) unchanged. Not pre-approved: R200 (Command routing/discovery), R204 (root redirect, composition).
- **F. RED plan:** `tests/ui/test_unified_shell_r199.py` fails at the opening sha on: brand strings absent, `command.css` still carrying `#7dd3fc`/`#38bdf8` and lacking the shared `--accent`, no cross-links, `/app` `refreshModels` not reading `providers`, no truth-strip element. GREEN after R199-A..E; the three moved title pins go GREEN in the same commit.
- **G. Exit:** gate of record on a fresh clone (`RESULT: PASS`, gateway 194), regression 18 roots, UI guards 73 / 12 / 22, freeze `--check` MATCHES, `changes_used 0`, ratchet `min_passed` (D5), PR by merge commit, post-merge gate, records-only closure PR, STOP at the next decision gate (R200 proposal requires a fresh `APPROVE`).

### R199-DEC-02 — R199 CLOSED: Unified Shell Foundation merged (one brand per surface, shared tokens in Command, cross-links, /app provider truth); UI-only, ceiling 0 used 0 (2026-09-21)
- **A. Operator text (verbatim):** "APPROVE R199. Accept D1–D5 as proposed: D1 = A, D2 = A, D3 = QEVION · Command / QEVION · Workbench / QEVION · Admin, D4 = use only real catalog/API provider-state values, D5 = standard gate ratchet. Execute R199 exactly as proposed. Do not pre-approve R200 or R204. Preserve the unified shared multi-tenant product direction and all existing security, authorization, Core, contract, and provider boundaries. Stop at the next decision gate." Executed exactly within D1–D5.
- **B. Delivered:** R199-A brand strings in `<title>` + sign-in `<h1>` of `ui/app`, `ui/app/command`, `ui/admin` (descriptor line "QEVION Control Plane" kept); three Command title pins moved. R199-B `command.css` carries the shared `:root` (identical to `ui/app`/`ui/admin`); Command-local names aliased; `#7dd3fc`/`#38bdf8` removed; motion untouched. R199-C `<a href>` cross-links between `/app/`, `/app/command/`, `/admin/` in every header, admin-only tagged, zero `/v1/` in hrefs. R199-D `/app` Models renders served `providers[]` per model and a truth strip (`models · providers · availability`) from the same `GET /v1/models` response only. R199-E Command sign-in copy names the admin requirement and links tenant users to the Workbench; `command.js` byte-identical.
- **C. Guards:** `tests/ui/test_unified_shell_r199.py` (22 items) — RED 17 failed / 5 passed on the opening tree (`evidence/r199/red.txt`) → GREEN (`evidence/r199/green.txt`); real-browser probe `evidence/r199/browser_probe_1607ee75.{json,py.txt}` + two screenshots (truth strip `models 1 · providers local_echo · availability available 1` == served response; Command `--brand` → `#4fd6ff`).
- **D. Invariants held:** `core/ apps/ infrastructure/` diff EMPTY (`round_r199` 0/0); `ui/admin/app.js` 73 = N0 and `command.js` 12 byte-identical to main; `ui/app/app.js` 22 / fetch 4; freeze `--check` MATCHES (46); no `/v1/` in any css; one execution path, one Agent authority, one Router untouched; root `/` still 404 (D2 = A).
- **E. Gates:** gate of record fresh clone `d8f39043` PASS **3951/0/0/64** + gateway 194; head re-gate `bdb3bb6e` PASS with the ratchet in force; regression 18 roots at d8f39043 0 failed (21 skipped, 1 xfail); D5 ratchet `min_passed` 3929 → 3951 (+22); PR #56 merged by merge commit → **`main 8110179e`**; post-merge fresh clone `8110179e` PASS 3951/0/0/64 + gateway 194 (`evidence/r199/gate_merge_8110179e.txt`, `gateway_merge_8110179e.txt`).
- **F. Incidents recorded honestly:** two sandbox resets during the round erased unpushed work (first RED commit + HTML edits; later the venv/browser cache) — redone from the pushed checkpoint each time; two inherited-guard trips during GREEN (word "three" in an HTML comment; `/v1/` in a css comment) fixed by wording (ledger rows 2–3, 7).
- **G. NOT claimed:** Production Ready (D-03 / N-9 OPERATOR-OWNED-OPEN); non-admin Command posture; capability→surface routing; `project_id` carry-through; `templates/{ref}` detail; shared agent panel; root redirect. **R200–R204 are NOT pre-approved** — each requires its own `APPROVE <ROUND_ID>`.
- **H. Next safe continuation point:** `main = 8110179e` + this records-only closure PR. Next session starts from main, reads `R199_POINTER` and ledger row 9, and does NOT start any round without an operator `APPROVE`.

### R200-DEC-01 — R200 opened: Command capability → surface routing + non-admin Command posture (tenant centre); UI-only, ceiling 0 (2026-09-22)
...
- **B. Findings (read-only seam verification at `main f6991e16`):** catalog `{id,state,evidence}` × 24, admin-gated (`apps/api/admin.py:1354–1364`; non-admin 403 measured); 19/24 evidence strings carry route pointers, 5 carry seam names only; `command.js` `loadCenter` hard-fails without `/v1/admin/capabilities`; `probeSession` rejects non-admins; node click ends in `#node-detail` (no navigation); no URL state in any tree (`/app` `showView(name)`, `/admin` `.rail-item[data-surface]`); guard bans quoted `CAPABILITY_IDS` in `command.js`; tenant-readable reads measured 200 for non-admin: `/v1/models /v1/templates(+/{ref}) /v1/executions /v1/usage /v1/workspaces /v1/skills /v1/webhooks /v1/agent-tools /healthz /v1/auth/session`; `/v1/agent/*` 403 for non-admin.
- **C. Rulings applied:** D1 = B — two-tier Command: tenants get a centre built only from tenant-readable reads; admin topology shown as ONE locked node labelled from `session.is_admin`; no admin data, no widened routes, no client-side gate emulation. D2 = i — routing derived from the served evidence route prefix (no capability id literals; no JSON asset). D3 = i — boot-once hash receivers `#view=` (`/app`) and `#surface=` (`/admin`), no `hashchange`, no timers. D4 = defer — no template picker in Command (13th literal refused; Ask links to `/app/#view=home`). D5 — affordance semantics as proposed (nothing hidden).
- **D. Decision:** open R200 by declaration as a **UI-only round (layer A/B), ceiling 0**. Allowed: `ui/app/command/{command.js,index.html,command.css}`, `ui/app/app.js`, `ui/admin/app.js`, `tests/ui/test_command_routing_r200.py` (RED first), pins that read "byte-identical" for `ui/admin/app.js`/`command.js` where they become count pins (12 / 73 / 22 / 4 all HOLD as numbers), `evidence/r200/*`, records. FROZEN: `core/ apps/ infrastructure/`; contracts + freeze (46); `CAPABILITY_IDS`; every `/v1/admin/*` and `/v1/agent/*` gate; register §C (no route newly consumed).
- **E. Invariants:** one execution path, one Agent authority (admin), one Router; the server 403 is the only permission authority — the UI labels, never decides; tenant path issues **zero** requests to `/v1/admin/*` or `/v1/agent/*` (browser-proved by request-set assertion); admin path request set identical to R185; no fabricated vocabulary (R182 §9) — "locked" is a permission FACT from `session.is_admin`, never a runtime state; `command.js` 12 literals, one `fetch(`.
- **F. RED plan:** `tests/ui/test_command_routing_r200.py` fails on the opening tree: no evidence-derived href construction; `probeSession` still requires `is_admin`; no tenant centre markup; no `#view=` in `app.js`; no `#surface=` in `admin/app.js`. GREEN after R200-A/B/C.
- **G. Exit:** gate of record fresh clone (`RESULT: PASS`, gateway 194), regression 18 roots, UI guards 73 / 12 / 22 / 4, freeze `--check` MATCHES, `changes_used 0`, ratchet `min_passed`, PR by merge commit, post-merge gate, records-only closure PR, STOP (no future round pre-approved).

### R200-DEC-02 — R200 CLOSED: Command capability → surface routing + tenant-tier Command merged; UI-only, ceiling 0 used 0 (2026-09-22)
- **A. Operator text (verbatim):** "APPROVE R200. Confirm: - D1 = B - D2 = I - D3 = I, including the /admin receiver - D4 = defer - D5 = accept as proposed. Execute R200 exactly as proposed. Preserve the unified shared multi-tenant QEVION direction: Command is the unified entry point, admin-only capabilities remain protected by the server, nothing implemented may become hidden, and no duplicate engine/router/agent or backend authority may be introduced. Do not pre-approve future rounds or broaden scope. Stop at the next decision gate."
- **B. Delivered:** R200-A `ui/app/command/command.js` — `SURFACE_BY_SEGMENT` / `ADMIN_SURFACE_BY_AREA` tables + `surfaceForEvidence()` (served evidence route parsed by an escaped regex → `{tree, hash, admin}`; `/healthz` special-cased; explicit `{none, reason}` fallback); `renderAffordance()` in node detail (link / `aria-disabled` "— admin" / "not linked: node is <state>" / reason line — nothing hidden); `probeSession` admits any authenticated session; `loadCenter` two-tier by `session.is_admin` — admin path identical to the R185 request set, tenant path `/healthz` + `refreshExecutions()` only + `renderTenantSurfaces()` + ONE locked admin node labelled from `session.is_admin`. R200-B boot-once hash receivers `applyDeepLink()` in `ui/app/app.js` (`#view=`, VIEWS-checked) and `ui/admin/app.js` (`#surface=`, rail click) — no `hashchange`, no timers. R200-C `index.html` `#tenant-view`/`#detail-surface`/panel ids; `command.css` affordance + tenant styles on shared tokens.
- **C. Guards:** `tests/ui/test_command_routing_r200.py` (21 items) — RED 14 failed / 8 passed on the opening tree (`evidence/r200/red.txt`), pin corrected before GREEN (evidence route parsed by regex, never a quoted `/v1/`), GREEN 21/21 at 2744fde0 (`evidence/r200/green.txt`). Real-browser proof `evidence/r200/browser_probe_2744fde0.json` + `command_tenant.png`, `command_admin.png`, `admin_deeplink_catalog.png`: tenant request set contains no `/v1/admin/*` and no `/v1/agent/*`; admin request set == R185; deep link `/admin/#surface=catalog` lands on catalog.
- **D. Invariants held:** `core/ apps/ infrastructure/` diff EMPTY (`round_r200` ceiling 0 used 0); `command.js` `/v1/` 12 + one `fetch(` in `api()`; `ui/app/app.js` 22 / 4; `ui/admin/app.js` 73 = N0; no `/v1/` in HTML/CSS hrefs or styles; `CAPABILITY_IDS` not quoted (R182 guard); no fabricated vocabulary (R185 guard); freeze `--check` MATCHES (46 served routes). Every `/v1/admin/*` and `/v1/agent/*` gate untouched — the server 403 is the only permission authority; the UI labels, never decides.
- **E. Gates:** gate of record fresh clone `c57d4c88` PASS **3972/0/0/64** + gateway **194** (`evidence/r200/gate_head_c57d4c88.txt`); ratchet `min_passed` 3951 → 3972 (+21) recorded at e434f76d and re-gated PASS; PR #58 merged by merge commit (mergeable_state clean, protection 404, statuses 0, check-runs 0) → `main ebe5e549`; post-merge fresh clone `ebe5e549` PASS **3972/0/0/64** + gateway 194 (`evidence/r200/gate_merge_ebe5e549.txt`, `gateway_merge_ebe5e549.txt`); regression 18 roots **3139 passed / 13 skipped / 0 failed** (`evidence/r200/regression_ebe5e549.txt`). `last_measured` → `ebe5e549 3972`.
- **F. Incidents recorded honestly:** ≥8 sandbox resets during the round erased unpushed edits, the venv/browser cache and several gate/regression outputs — each time work resumed from the pushed checkpoint (commit + push per edit; self-recording background gate scripts adopted). The first merge call for PR #58 was interrupted with an unknown result; the API was re-checked (PR still open, `clean`) before the merge was re-issued — no duplicate merge. Ledger rows 2, 5–7.
- **G. NOT claimed:** Production Ready (D-03 / N-9 OPERATOR-OWNED-OPEN); template picker in Command (D4 deferred — no 13th `/v1/` literal); `templates/{ref}` detail; `project_id` carry-through; shared agent panel; root `/` redirect; any register §C flip; any token sharing across trees.
- **H. Next safe continuation point:** `main = ebe5e549` + this records-only closure PR. Next session starts from main, reads `R200_POINTER` and ledger row 9, and does NOT start any round (R201+) without an operator `APPROVE`.

### R201-DEC-01 — R201 opened: Context-carrying execution — execution id carried Command → Workbench/Admin; Workbench self-restores its selection; UI-only, ceiling 0 (2026-09-22)
- **A. Operator text (verbatim):** "APPROVE R201 with D1–D7 confirmed (recommended: D1=a, D2=i, D3=i, D4=defer, D5=defer, D6=defer, D7=defer)"
- **B. Findings (read-only seam verification at `main daa88afd`):** `POST /v1/execute` accepts `project_id` for ADMISSION only (tenant-scoped resolve, one 404 body, audit) — the execution record carries NO project (`core/contracts/execution.py:76–94`; `executions` table has no `project_id`); `GET /v1/executions` filters are `status, initiated_by, created_after, created_before, limit` only — `?project_id=` is silently ignored (measured 200 unfiltered); `GET /v1/projects?workspace_id=` IS supported (422 on non-UUID, measured). Session `{user_id, tenant_id, email, is_admin}` is server-derived. Command holds `selectedExecution` + `executions[]` and NO workspace/project/template state; Workbench holds `selectedWorkspace`, `#ask-project`, `#ask-template` in memory only; both receivers use anchored regexes (`^#view=([a-z]+)$`, `^#surface=([a-z]+)$`) so nothing but the view survives a hop; existing `openRun(id)` / `openExecution(id)` are the natural receivers. No UI holds a model selection (`model_policy` unused). The hash survives the login form in both trees.
- **C. Rulings applied:** D1 = a — carry the execution id in the hash (`&execution=<uuid>`); receivers call the EXISTING open-by-id functions; the server's 404/422 is shown verbatim. D2 = i — Workbench self-restores non-secret selection (`sessionStorage` `qevion.app.context`, R187 pattern; never the token, never `localStorage`; served-list membership required; explicit hash wins). D3 = i — grammar carries `&execution=` only; no `project=`/`template=` receiver without an emitter. D4 = defer — token custody across trees is a security-posture round. D5 = defer — project filter / project on execution is architectural (contract + schema + store + repos + route). D6 = defer — no model selection exists. D7 = defer — Command attaches no `project_id` (would add `/v1/` literals).
- **D. Decision:** open R201 by declaration as a **UI-only round (layer A/B), ceiling 0**. Allowed: `ui/app/command/{command.js,index.html,command.css}`, `ui/app/app.js`, `ui/admin/app.js`, `tests/ui/test_context_carry_r201.py` (new, RED first), `evidence/r201/*`, records. Frozen: `core/ apps/ infrastructure/` (0); contracts + freeze 46; all `/v1/admin/*` + `/v1/agent/*` gates; `command.js` 12 + one `fetch(`; `ui/app/app.js` ≤ 22 / 4; `ui/admin/app.js` 73; register §C; `CAPABILITY_IDS`.
- **E. Invariants:** one execution path, one Agent authority, one Router; the server is the only tenancy/permission authority — the UI carries an id, never decides ownership; receivers boot-once (no `hashchange`, no timers, no new reads); nothing hidden, nothing fabricated (links built only from served rows / served-mount hrefs).
- **F. RED plan:** `tests/ui/test_context_carry_r201.py` fails on the opening tree: no `executionHref`; tenant tier lists no execution links; admin record has no surface affordances; receivers reject `&execution=`; app.js has no `qevion.app.context` / `sessionStorage`; `round_r201` present with ceiling 0.
- **G. Exit:** GREEN + real-browser proof (tenant hop opens the record with exactly one `GET /v1/executions/<id>` and no `/v1/admin/*`; admin hop; malformed id issues no request; foreign id shows the server's verbatim 404; round trip restores workspace/project/template; `localStorage` empty), gate of record fresh clone (`RESULT: PASS`, gateway 194), regression 18 roots, counts 12/1 · ≤22/4 · 73, freeze `--check` MATCHES, `changes_used 0`, ratchet `min_passed`, PR by merge commit, post-merge gate, records-only closure PR, STOP.

### R201-DEC-02 — R201 CLOSED: Context-carrying execution merged (execution id carried Command → Workbench/Admin; Workbench self-restores selection); UI-only, ceiling 0 used 0 (2026-09-22)
- **A. Operator text (verbatim):** "APPROVE R201 with D1–D7 confirmed (recommended: D1=a, D2=i, D3=i, D4=defer, D5=defer, D6=defer, D7=defer)"
- **B. Delivered:** R201-A `ui/app/command/command.js` — `ADMIN_EXECUTIONS_OWNER` (one named table entry) + `executionHref(owner, id)` → `<owner.href>&execution=<encoded uuid>` (no `/v1/` literal; 12 + one `fetch(` hold); `renderExecutionSurfaces(id, session)` called from `showExecution` renders "Open Workbench · runs" and "Open Admin · executions" — admin target visible, `aria-disabled` + labelled from `session.is_admin` for a non-admin session; `renderTenantSurfaces` lists every served `state.executions` row (id, status, created_at as served) as a Workbench link — no second read. `index.html` `#tenant-execution-list`, `#execution-surfaces`; `command.css` styles on shared tokens. R201-B `applyDeepLink` (`ui/app/app.js`, `ui/admin/app.js`) accepts an optional `&execution=<uuid>` (anchored regex; malformed → whole hash ignored) and calls the EXISTING `openRun(id)` (runs only) / `openExecution(id)` (executions only); no `hashchange`, no timers, no new reads. R201-C `ui/app/app.js` `enterMain` awaits `refreshWorkspaces()` + `populateTemplateSelect()` then `restoreContext()` then `applyDeepLink()`; `saveContext` on `showView`, `selectWorkspace`, project/template change, "Use in composer"; `clearContext` on logout; key `qevion.app.context` = {view, selectedWorkspace, project, template} — never the token; `localStorage` absent.
- **C. Guards:** `tests/ui/test_context_carry_r201.py` (12 items) RED 8 failed / 4 passed on the opening tree 109fe293 (`evidence/r201/red.txt`) → GREEN 12/12 (`evidence/r201/green.txt`); static UI + composition 205/0/0. Real-browser proof (`evidence/r201/browser_probe.py.txt`, `browser_probe_b67c0a09.json`, 7 screenshots): tenant Command lists `/app/#view=runs&execution=<id>`; the hop opens the record with EXACTLY one `GET /v1/executions/<id>` and no `/v1/admin/*`/`/v1/agent/*`; admin record links `/app/#view=runs&execution=<id>` + `/admin/#surface=executions&execution=<id>`; the admin deep link lands on the executions rail with the record; malformed id → no by-id request, runs view not shown; **tenant B with tenant A's id → `validation_error: Unknown execution id.` verbatim from the server, record hidden** (no client tenancy logic); round trip restores workspace + project + template + view `usage`; `#view=models` overrides the stored view; `localStorage` keys `[]`; context `null` after logout.
- **D. Invariants held:** `core/ apps/ infrastructure/` diff EMPTY (`round_r201` 0/0); `command.js` 12 / 1; `ui/app/app.js` 22 / 4; `ui/admin/app.js` 73; no wired `/v1/` in command html/css; R182/R185/R197/R199 guards GREEN; freeze `--check` MATCHES (46). The server's 403/404/422 remain the only permission/tenancy authority — the UI carries an id, never decides ownership.
- **E. Gates:** gate of record fresh clone `798903ca` PASS **3984/0/0/64** + gateway **194** + regression 18 roots **3139 passed / 13 skipped / 0 failed** (`evidence/r201/gate_head_798903ca.txt`, `gateway_head_…`, `regression_head_…`); ratchet `min_passed` 3972 → 3984 (+12); head re-gate `318d7930` PASS 3984/0/64 with the ratchet in force; PR #60 merged by merge commit (clean / protection 404 / statuses 0 / check-runs 0) → `main 2140e541`; post-merge fresh clone `2140e541` — first run **FAIL 3980/4** (ledger row 10, evidence kept): the four real-browser tests (r182/r185/r186/r187) could not launch Chromium because the sandbox reset preceding the merge erased `/home/user/.cache/ms-playwright` (`BrowserType.launch: Executable doesn't exist`); after `playwright install chromium` the same four tests pass alone (18 passed) and the full gate on the SAME clone/sha re-ran **PASS 3984/0/0/64** (row 11, `gate_merge_2140e541_rerun_browser_restored.txt`); gateway 194; regression 3139/13/0. `last_measured` → `2140e541 3984`.
- **F. Incidents recorded honestly:** three sandbox resets during the round (after writing the guard; during the PR #60 merge call — API re-check showed the PR still open before a single re-issue; before the post-merge gate — the erased browser binary produced row 10's FAIL). One incomplete ratchet commit (`fc6275c4`, script aborted on a string-vs-list anchor) was corrected in `318d7930`; the gate script's missing `bc` left row 5's sums blank, recomputed from the slice lines in row 5's amendment. Nothing was overwritten; every incident is in the ledger.
- **G. NOT claimed:** Production Ready (D-03 / N-9 OPERATOR-OWNED-OPEN); D4 token custody across trees (memory → re-login stays); D5 project filter on `GET /v1/executions` / project on the execution record (architectural: contract + schema + store + repos + route); D6 model/provider selection; D7 Command attaching `project_id`; `templates/{ref}` detail; root `/` redirect; any register §C flip; any token sharing across trees.
- **H. Next safe continuation point:** `main = 2140e541` + this records-only closure PR. Next session starts from main, reads `R201_POINTER` and ledger row 12, and does NOT start any round (R202+) without an operator `APPROVE`.

### R202-DEC-01 — R202 opened: Templates + App Factory UX — the served template detail becomes visible before a run; UI-only, ceiling 0; ui/app/app.js /v1/ ceiling 22 → 23 (2026-09-22)
...
- **B. Findings (read-only seam verification at `main 22111694`):** ONE `TemplateRegistry` composes exactly one system template `app_factory.plan@1` (3 stages inventory-summary → architecture-plan → review; roles analyst/architect/reviewer; `max_parallel 1`; description states generation is a later, separate template). `GET /v1/templates` (list, R196) is consumed by the R197 picker; `GET /v1/templates/{ref}` serves the FULL `StrategyTemplate` (stages with kind/role/instruction/depends_on) tenant-authenticated, one 404 for unknown/non-system — **DECLARED-UNCONSUMED** (register §C) since R197 D4. `POST /v1/execute` `mode=template` accepts `id@version` and bare `id`; unknown → 422 on `execution_strategy.template_id`; runs through the ONE `StrategyExecutor` (hybrid record, one node per stage, SSE `node_started/node_completed` per stage). `StrategyExecutor.resolve` returns a template VERBATIM — a request `model_policy` is accepted by the contract but has NO effect in template mode; all App Factory stages carry `model_policy: null` ⇒ Router AUTO per stage; `TemplateOverride` exists in core but no route carries it. `AppFactoryCapability` is not composed; `/v1/agent/converse` is admin-only and plans `strategy: single` — the only tenant-reachable App Factory path is `/v1/execute` template mode; `generation_deferred` is always true. Command holds no template state (routing string only). D4–D7 of R201 verified as NON-blockers for this UX (custody: single-tree flow; project filter and template-ref-on-record: architectural; model selection: would be silently ignored today, so the UI must show AUTO and offer no selector; Command picker: would breach the 12-literal down-only rule and duplicate the composer).
- **C. Rulings applied:** D1 = a — consume `GET /v1/templates/{ref}` in the Workbench detail panel. D2 = i — one read per choice on `#ask-template` change (and after a restored choice), never at module load, none for the empty choice; `ui/app/app.js` `/v1/` ceiling re-measured **22 → 23** for this ONE literal (declared here, before code; down-only after). D3 = i — render the template as served; no second surface; no code-gen affordance. D4 = i — async timeline rows labelled `kind · role` from the loaded detail (no new read). D5 = i — Command `templates` href unchanged; no Command template read; no receiver without an emitter. D6 = i — show the served AUTO posture; no selector; `TemplateOverride` wiring deferred to its own round. D7 = yes — register §C flip at closure with evidence. D8 = defer — template ref on the execution record is backend/contract work.
- **D. Decision:** open R202 by declaration as a **UI-only round (layer A/B), production ceiling 0**. Allowed: `ui/app/{app.js,index.html,styles.css}`, `tests/ui/test_template_detail_r202.py` (new, RED first), the R197 D4 pin flipped 1:1 in `tests/ui/test_workbench_template_picker_r197.py`, R201 count pin `<= 22` → `<= 23` in `tests/ui/test_context_carry_r201.py`, manifest ceiling fields, `evidence/r202/*`, records. Frozen: `core/ apps/ infrastructure/` (0); contracts + freeze 46; all `/v1/admin/*` + `/v1/agent/*` gates; `command.js` 12 + one `fetch(`; `ui/admin/app.js` 73; `CAPABILITY_IDS`; `POST /v1/execute` wire shape (`execution_strategy` only when a ref is chosen — R197 pin holds).
- **E. Invariants:** one Router, one StrategyExecutor, one TemplateRegistry, one execution path; the server is the only authority (404 verbatim); nothing hidden, nothing fabricated (every rendered field is a served field; "auto (Router decides)" is derived from the served `model_policy: null`); no provider names; no hardcoded template id/ref (`app_factory` literal stays banned in app.js).
- **F. RED plan:** `tests/ui/test_template_detail_r202.py` fails on the opening tree: no `loadTemplateDetail` / `renderTemplateDetail`; no `#template-detail` markup; `followEvents` has no stage labels; app.js `/v1/templates/` count 0 (must be exactly 1); manifest ceiling 23 present.
- **G. Exit:** GREEN + real-browser proof (one detail read per choice; panel content == served detail; none for "none"; restored choice → one read; unknown ref → server 404 verbatim; async run → labelled stage rows; `localStorage` empty), gate of record fresh clone (`RESULT: PASS`, gateway 194), regression 18 roots, counts 23/4 · 12/1 · 73, freeze `--check` MATCHES, `changes_used 0`, ratchet `min_passed`, PR by merge commit, post-merge gate, records-only closure PR incl. register flip, STOP.

### R202-DEC-02 — R202 CLOSED: served template detail merged (R202-A ACCEPTED-AS-MEASURED); R202-B DEFERRED per FINDING R202-F1; UI-only, ceiling 0 used 0 (2026-09-23)
- **A. Operator (verbatim, ruling at the decision gate):** "RULING: (a) ACCEPT R202-A AS THE PROVEN R202 SCOPE. R202-A = accepted-as-measured. R202-B is DEFERRED and remains latent only: keep stageLabel/followEvents code as-is; 0 production literals; 0 reads; statically guarded; do not claim R202-B as reachable/proven; preserve R202-F1 / R202-DEC-02 as authoritative evidence. Do NOT modify R188 C3. Do NOT add async template execution. Do NOT change execution semantics. Do NOT add backend production files. Do NOT widen the R202 scope. Proceed: 1. ratchet min_passed 3984 -> 3993 2. re-gate HEAD 3. open PR 4. merge by merge commit 5. run fresh post-merge gate 6. execute R202 closure PR 7. flip register §C only for the proven R202-A scope 8. record R202-B as deferred due to R202-F1. Preserve: unified QEVION architecture; one Router; one StrategyExecutor; one TemplateRegistry; existing execution path; additive-only / no-regression discipline. Keep: app.js /v1/ ceiling = 23; core/apps/infrastructure production diff = 0 for R202. STOP at the next decision gate."
- **B. Delivered (R202-A, measured):** `ui/app/app.js` `loadTemplateDetail(ref)` → ONE `GET /v1/templates/{ref}` per choice through the existing `api()` (on `#ask-template` change and after a restored R201-C context; never at module load; none for the empty choice); `renderTemplateDetail` renders the served `StrategyTemplate` verbatim (name · id@version · origin/status; description; `tag:/skill:/requires:` chips; one row per `strategy.stages[]` with key/kind/role/depends_on/model posture/instruction; served `model_policy` null → "auto (Router decides)"); zero controls in the panel; server errors verbatim (`validation_error: unknown template.`). `#template-detail` markup starts hidden; CSS additive. R197 D4 pin flipped 1:1 to "consumed exactly once"; R199/R200/R201 count pins → 23.
- **C. FINDING R202-F1 (authoritative; recorded, not worked around):** `POST /v1/execute` refuses `execution_strategy` + `execution_policy.async` with 422 (`apps/api/app.py` R188 C3, "execution_strategy runs synchronously in this slice"); template runs are therefore always sync 200 and the Workbench opens the SSE stream only after a 202 ⇒ **R202-B** (`stageLabel` in `followEvents`, D4 = i) is not reachable through the shipped UI in this slice. The proposal's exit premise "async run → labelled stage rows" was wrong (assistant's error; not the operator's). The API-level SSE replay of the same execution DOES emit `node_started/node_completed` per stage (emitter exists). Real-browser evidence: `evidence/r202/workbench_template_async_422.png` (422 rendered verbatim, 0 timeline rows), `workbench_template_sync_200.png` (sync 200; no `/events` request). **Ruling (a):** R202-B DEFERRED — code kept latent (0 literals, 0 reads, statically guarded), never claimed reachable or proven; R188 C3 untouched; no async template execution; no execution-semantics change.
- **D. Measured:** RED `evidence/r202/red.txt` 7 FAILED on `114c6307` → GREEN 43/43 (`green.txt`); static UI+composition 205/0 (`static_suite_9a6f89cb.txt`); browser proof `browser_probe_9a6f89cb.json` + 4 PNGs; gate of record fresh clone `a4a8aad9` `RESULT: PASS` passed=3993 f=0 e=0 s=64, gateway 194, regression 18 roots 3139/13/0; ratchet `min_passed` 3984 → 3993 (D-6, +9 = `tests/ui/test_template_detail_r202.py`); head re-gate `7506ad42` PASS 3993; PR #62 merged by merge commit (`mergeable_state clean`, protection 404, statuses 0, check-runs 0) → `main 5ac352be`; post-merge fresh clone `5ac352be` PASS 3993/0/0/64, gateway 194, regression 3139/13/0. Counts: `ui/app/app.js` `/v1/` 23 (== declared ceiling) · `fetch(` 4 · `localStorage` 0; `command.js` 12 + one `fetch(`; `ui/admin/app.js` 73; freeze `--check` MATCHES (46); production diff `core apps infrastructure` 0 lines (`changes_used 0`).
- **E. Incidents (recorded):** (5a) the self-recording gate script summed the two summary `passed=` lines too and wrote 11979 (= 3 × 3993) into ledger row 5 text and the commit title of `cb27b6b3`; gate file untouched and authoritative; row corrected, script fixed. Probe drafts: URL-decoding of `%40` and a required re-login after the cross-app hop (token is memory-only) were probe defects, fixed before the recorded run; the `async+template` step was rewritten to record the served 422 rather than to work around it.
- **F. Records:** register §C `GET /v1/templates/{ref}` DECLARED-UNCONSUMED → ACCEPTED-AS-MEASURED **for R202-A only** (R202-B explicitly excluded); `R202_HANDOFF.md` CLOSED + §5; `R202_POINTER` CLOSED; `pytest.last_measured` → `5ac352be` / 3993; `round_r202.ceiling_history` gate entry; `evidence/r202_state_ledger.md` rows 1–10.
- **G. Invariants held:** one Router, one StrategyExecutor, one TemplateRegistry, one execution path; server-side tenancy/authority (404/422 verbatim); nothing hidden, nothing fabricated; additive-only; `/v1/execute` wire shape unchanged (`execution_strategy` only when a ref is chosen; no `model_policy`, no `template_override`).
- **H. Not claimed / open:** Production Ready (D-03/N-9 operator-owned-open); R202-B reachability; async template execution (would need a backend round touching R188 C3 — not proposed, not approved); D6 TemplateOverride/model selection; D8 template ref on the execution record; R201 D4/D5/D7. **No R203 pre-approved.**

### COMPLETION-V2-DEC-01 — COMPLETION PROGRAM v2 opened AND executed as ONE round from main 7d9524eb: three phases (C-01..C-17), operator rulings D-1..D-7; production ceiling 13; ui/app/app.js ceilings 23/4 → 32/6 by declaration (2026-09-24)
- **A. Operator (verbatim rulings):** "D-1 = b / D-2 = HttpOnly cookie session / D-3 = yes / D-4 = yes / D-5 = close the bounded governed learning loop / D-6 = yes / D-7 = do not block the completion work on unavailable live credentials; complete everything verifiable hermetically and mark live-provider success NOT VERIFIED until a real eligible provider can actually return 200." — "Execute ALL THREE phases as ONE PRODUCT COMPLETION PROGRAM … Do NOT reinterpret these as separate future engineering rounds." — reuse existing authorities (no second Router / StrategyExecutor / agent runtime / TemplateRegistry / SkillRegistry / ToolExecutor / capability registry / provider manager / credential / tenancy); USE→COMPOSE→WIRE→REPAIR→SIMPLIFY→ADD; approval not required for ordinary wiring/fixes/tests/browser verification; "If this cannot be achieved with the existing authorities, do not fake it."
- **B. Delivered — PHASE 1 (C-01..C-09):** C-01 `GET /` → 307 `/app/` (`apps/composition/runtime.py`). C-02 (D-1 = b) `POST /v1/auth/register` on the in-memory profile returns `verification:"dev_token"` + `dev_verification_token` + `dev_note` — the `ConsoleEmailSender(capture=)` binds the capture ONLY when `durable` is false; the durable profile's response is byte-identical to before; the Workbench pre-fills the token and repeats the server's label. C-03 readable result: answer + fact chips (mode / strategy / template / project / provider / verbatim local-echo note / context blocks / gold blocks / memory sources) with raw JSON one click away. C-04 (D-4) additive `context {strategy, mode, project_id, template_ref}` on sync/status/list from the REQUEST spec + stored record (`apps/api/execution_context.py`; folded into `cost_snapshot.request_context` by the sync path and by the worker from the outbox payload). C-05 (D-2) login sets `qevion_session` HttpOnly SameSite=Strict; `session_token()` = Bearer first, cookie second; cookie-only state changes require `X-Requested-With: QEVION` (403 `unauthorized` verbatim otherwise); logout clears the cookie; the Workbench stores NOTHING in browser storage (`state.token` stays null). C-06 a FAILED template/custom stage node names `stage` + the child's normalized `provider_error_category`/`safe_message`; the attempt trail rides the projected node so `execution_failure_detail` maps the REAL category and adds `details.stage`. C-07 orientation + first-run block. C-08 idempotent workspace select + Close. C-09 `ERROR_HINTS` (11 unified codes) → one "Next:" line under the verbatim error.
- **C. Delivered — PHASE 2 (C-10..C-13):** C-10 template runs replay the stored SSE log once → stage rows `key · kind · role` (R202-B's latent `stageLabel` now has a REAL emitter; R188 C3 untouched — template runs stay sync). C-11 (D-3) `GET /v1/agent/executions/{id}/trace|diagnosis` admit any authenticated caller and resolve in the caller's tenant (foreign == absent == 404); `/tools` + `/converse` stay admin-only; the Workbench offers the agent toggle only when `GET /v1/agent-tools` serves ≥1 tool, else disabled + "unavailable" (hermetic: 0 tools ⇒ honestly disabled); "Agent trace" on the run view for own agent runs. C-12 `/v1/models` carries per-binding `runtime {provider, binding_availability, eligible, reason, retry_after_ms}` from the ONE `ResourceSignalBoard` (`SimpleScoringRouter.signals`); DEGRADED/UNAVAILABLE derived; `ModelListEntry.runtime` additive. C-13 (D-6) read-only Capabilities view over `GET /v1/skills`, `/v1/memory/preferences` (+ DELETE), `/v1/webhooks` — 404 ⇒ "not composed", errors verbatim, empty states honest.
- **D. Delivered — PHASE 3 (C-14..C-17, D-5):** the loop is CLOSED over HTTP with the existing authorities only: `/v1/execute` → `POST /v1/admin/learning/samples {source_execution_id}` (C-14; foreign execution ⇒ 404) → `/evaluate` (ONE record with deterministic + SECURITY grader rows — the lifecycle's `EvaluationPolicyService` composition) → `/scan` → `/sanitize` → `/admit` → `POST /v1/admin/scenarios` + `/replay` (REAL labelled execution + evaluation record) → `/promote {evidence_refs{evaluation_id, security_evaluation_id (same record), regression_execution_id}}` resolved by `PromotionEvidenceResolver` (strict; self-asserted passes REFUSED `promoted:false`) → GOLD `MemoryItem source=learning.gold` → later `/v1/execute` → `context_provenance.gold_blocks == 1` on the sync body AND the run view (C-16); another tenant's identical ask stays `gold_blocks 0`. C-15 admin console Promote no longer asserts artefact passes: it hands over HARVESTED refs (Evaluate's `evaluation_id`, the last PASSED Replay's `execution_id`) and asserts only the human conditions. C-17 `GET /v1/admin/learning/dashboard` is MEASURED (`LearningDashboardMeasured`: samples by level, verified/gold/eligible, learned_keys, evaluations_recorded, promotion_history from `TRAINING_DATASET_PROMOTED` audit events; `deferred` NAMES the 8 unmeasured metrics) — the console renders it; `/learning/learned` swapped 1:1 (73 literals held).
- **E. Measured:** `tests/completion_v2/` 17 items over the REAL runtime profile (`test_phase12_runtime.py` 11, `test_phase3_learning_loop.py` 3, `test_c06_stage_failure.py` 3); HTTP probes `evidence/completion_v2/probe_api_phase12.json`, `probe_learning_loop.json`; REAL Chromium journey A–J against `python3 -m apps.main` — `evidence/completion_v2/browser/facts.json` + 9 PNGs (dev-token onboarding; `qevion_session` httpOnly/Strict; `localStorage`/`sessionStorage` EMPTY; reload keeps the session; answer + facts; runs + detail; template stage replay 3 stages; error hint; model runtime rows; caps honest; agent toggle disabled "no tools offered"; admin capture→…→Promote with harvested refs → dashboard gold 1 / promotions 1 → Workbench `gold blocks: 1 · memory sources: learning.gold`; other tenant 0; logout ⇒ auth view). Gate numbers: see COMPLETION-V2-DEC-02.
- **F. Findings fixed in-round (recorded):** (1) the admin console and Command `api()` sent no CSRF header — with the Workbench cookie present in the same browser their POSTs (incl. login) were refused ⇒ both now send `X-Requested-With` (73/12 literals held). (2) the Workbench banner claimed "Durable profile — … survive restarts" on the in-memory profile ⇒ it now repeats only what the server said. (3) the C-04 context must come from the REQUEST spec (the resolved plan reports `custom`). (4) 2 ruff-format-only drifts (`core/learning/storage.py`, `apps/composition/learning.py`) reverted to main bytes — not counted. (5) AA2 non-admin pin split per D-3; R160 learning-verdict pin updated per C-15; UI count pins flipped 1:1 with lineage.
- **G. NOT VERIFIED / deferred (D-7, honest):** live-provider success (a real provider returning 200) — every run in evidence is the hermetic local echo adapter, labelled verbatim "no real model was called"; a plan-refused/credential-refused provider is NEVER relabelled healthy. Agent path with tools NOT exercised in the browser (0 tools offered hermetically; the toggle is honestly disabled — proven by test, not by a live agent run). Dashboard `deferred` metrics (dataset_coverage, task_coverage, specialist_models, accuracy_trends, cost_reduction, teacher_agreement, canary_status, rollback_actions) remain unmeasured and are NAMED as such. Production Ready NOT claimed (D-03 / N-9 operator-owned-open unchanged).
- **H. Invariants held:** one Router / StrategyExecutor / TemplateRegistry / agent runtime / execution path; server-side tenancy (404/403 verbatim); additive-only contracts (freeze re-derived, declared); Bearer auth unchanged; nothing fabricated.

### COMPLETION-V2-DEC-02 — COMPLETION PROGRAM v2 CLOSED: PR #65 merged (merge commit b8195e3c); gate of record 0a25f476 PASS; post-merge gate PASS; product status recorded honestly (2026-09-24)
- **A. Merge:** `completion_v2` → `main` via PR #65 (merge commit `b8195e3c`, 29 commits, 74 files, +4313/−297; production roots 13 files = `round_completion_v2` 13/13).
- **B. Gates:** worktree `a400ec14` PASS 4016/0/0/58 (`evidence/completion_v2/gate_worktree_a400ec14.txt`); FRESH-CLONE gate of record `0a25f476` PASS 4016/0/0/58 (`gate_fresh_clone_0a25f476.txt`); POST-MERGE fresh venv on `main b8195e3c` PASS 4016/0/0/58 (`gate_post_merge_b8195e3c.txt`); mypy --strict / ruff / import-linter / secret scan 5/5 / change budget PASS in all three; `min_passed` ratcheted 3993 → 4016 at the gate of record (D-6 pattern); contract freeze MATCHES (round completion_v2).
- **C. Incident (recorded):** the first post-merge run after a sandbox reset reported `rest` 1611/4 — the four Playwright browser tests fail CLOSED when Chromium is absent from the fresh sandbox ("chromium not launchable … run playwright install"); after `playwright install chromium` the same tree passed 1615/0. Environmental, not a code regression; no change made.
- **D. FINAL ACCEPTANCE (15 points, measured):** (1) root entry 307 → `/app/` ✔ browser A; (2) dev onboarding without email ✔ A (labelled dev token, in-memory only); (3) session survives reload ✔ B (HttpOnly Strict cookie; browser storage empty); (4) real task runs ✔ B; (5) readable result ✔ B (answer + facts, raw one click away); (6) run context visible ✔ C/D (mode · template · project); (7) agent reachable when prerequisites exist ✔ by test C11 + honest disabled state in the browser (0 tools hermetically) — **live agent run NOT VERIFIED**; (8) template stages visible ✔ D (3 stages replayed from the stored record); (9) truthful model state ✔ E (runtime rows from the signal board; test C12 quota → unavailable); (10) actionable errors ✔ G (verbatim code + Next hint; C-06 `details.stage` by test); (11) tenant capabilities discoverable ✔ F (honest empty/not-composed states); (12) isolation intact ✔ (foreign trace 404, foreign capture 404, other tenant gold 0, admin 403 for non-admin; 29+ isolation tests green); (13) learning proven `gold_blocks > 0` ✔ H/I over HTTP + browser with evidence-backed promotion — NOT deferred; (14) no second authority ✔ (import-linter 13/0; one Router/StrategyExecutor/agent/TemplateRegistry; diff inspected); (15) no silent incompleteness ✔ — every unverified item is NAMED below.
- **E. NOT VERIFIED (D-7, unchanged):** live-provider 200 (all measured runs = hermetic local echo, labelled verbatim); live agent run with tools; dashboard `deferred` metrics (8, named in the payload); durable-profile cookie `Secure` flag (needs `SESSION_COOKIE_SECURE=1` behind TLS — not exercised); `not_evaluated` #1 unchanged.
- **F. FINAL PRODUCT STATUS:** *Hermetically complete and verified; live-provider path NOT VERIFIED.* Production Ready is **NOT claimed** (D-03 credential rotation/purge and N-9 branch hygiene remain OPERATOR-OWNED-OPEN; no deployment artefacts in repo per ADR-0014). Branch `completion_v2` may be deleted per the compare rule; **no follow-up round is pre-approved.**
