...
| `GET /v1/admin/learning/dashboard` | COMPLETION-V2 C-17: MEASURED when the lifecycle is composed (`LearningDashboardMeasured`; `deferred` names the 8 unmeasured metrics); placeholder only without a lifecycle; consumed by the admin console Learning surface (1:1 swap for `/learning/learned`, 73 held) | ACCEPTED-AS-MEASURED | `tests/completion_v2/test_phase3_learning_loop.py`; `evidence/completion_v2/browser/facts.json` (`H_dashboard_after`) |
| `GET /v1/agent/executions/{id}/trace`, `/diagnosis` (tenant) | COMPLETION-V2 C-11 (D-3): owning tenant reads its own trace/diagnosis from the Workbench run view; foreign id ⇒ 404 | ACCEPTED-AS-MEASURED | `tests/completion_v2/test_phase12_runtime.py` (C11); `tests/admin_agent/test_aa2_admin_agent.py` |
| `GET /v1/skills`, `GET /v1/memory/preferences` (+DELETE), `GET /v1/webhooks` (tenant Workbench) | COMPLETION-V2 C-13 (D-6): read-only Capabilities view; 404 ⇒ 'not composed', errors verbatim | ACCEPTED-AS-MEASURED | `evidence/completion_v2/browser/facts.json` (`F_caps`) |
...
| Learning loop (D-5) | COMPLETION-V2: execution → capture → evaluate → scan → sanitize → admit → scenario replay → promote(evidence_refs, strict) → GOLD → `gold_blocks` 0→1 on a later `/v1/execute` — proven over HTTP and in a real browser; other tenant stays 0 | `tests/completion_v2/test_phase3_learning_loop.py`; `evidence/completion_v2/probe_learning_loop.json`; `browser/facts.json` |
| Live provider (D-7) | NOT VERIFIED — every measured run is the hermetic local echo adapter (labelled verbatim); no real provider returned 200 in this program | `evidence/completion_v2/` |
