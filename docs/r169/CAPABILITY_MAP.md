...
`apps/api/capabilities.py` holds ONLY the closed id set (`CAPABILITY_IDS`, 24 ids as of R196 — 16 from V7 plus
...
| `templates.listing` (R196, AD-3) | `templates is not None` | `templates=` (`TemplateRegistry`) → **available**; the runtime composes `build_template_registry()` (the ONE built-in `APP_FACTORY_TEMPLATE`) | routes absent (404); `mode="template"` executions fail `UnknownTemplate` | `GET /v1/templates` → `TemplatesListResponse{templates:[TemplateListEntry]}` (ACTIVE SYSTEM only); `GET /v1/templates/{ref}` → full `StrategyTemplate` (unknown/inactive/non-system ⇒ 404 `validation_error`); tenant-authenticated | Let a UI/operator list and inspect system strategy templates and select one for `/v1/execute` `execution_strategy.mode="template"`. Workspace ownership = recorded later direction, not implemented. **UI consumer (R197):** Workbench `select#ask-template` over `GET /v1/templates`; template-mode execute proven live (`evidence/r197/workbench_template_picker_dom_probe.json`). |
...
| `webhooks.delivery` (R194-B; composition fact, not a served route) | `RuntimeProfile.webhook_worker` present | `runtime.py` passes ONE tenant-scoped subscription map to `create_app` **and** `ExecutionMessageHandler`; a second `Worker` on `WEBHOOK_STREAM` drives `WebhookDeliveryHandler` with an httpx sender (`follow_redirects=False`, `WEBHOOK_TIMEOUT_SECONDS`); `apps.main` runs it in the lifespan → **available** since R194 (before R194 registrations were accepted but nothing was ever delivered) | n/a | outbound `POST <registered url>` with the 10 §12 `WebhookPayload`; non-2xx ⇒ retry then dead-letter (40 §4.7) | Deliver `execution.queued` / terminal events to the owning tenant's subscriptions only (20 §6). |
| `dev.governed_bindings` (R195, AD-1; admin lifecycle, not a `/v1/dev` route) | `AdminConfigService(repo_bindings=, remote_trust=)` bound ⇔ `AGENT_DEV_STATE_DIR` set | `AdminAction.REGISTER_REPO_BINDING` / `GRANT_REMOTE_TRUST` / `REVOKE_REMOTE_TRUST` (area `tools`) ride `POST /v1/admin/changes` → validate → preview → publish → rollback over the SAME `RepoBindingRegistry` / `RemoteTrustRegistry` the git tools read → **available** since R195 | seams absent ⇒ the three actions fail validation loudly ("… seam is not bound in this composition") | discovered on `GET /v1/admin/capabilities/actions` | Register a tenant's repo binding and grant/revoke remote trust as governed, audited, reversible admin acts (G-BIND / G-TRUST) |
...
| `dev.publish_modes` (R172 C7; composed R193) | `dev_bindings is not None` | `dev_bindings=` (`apps.agent_dev.git_tools.RepoBindingRegistry`) — injected by `runtime.py` **only when `AGENT_DEV_STATE_DIR` is set** (`apps/composition/dev_bindings.py`, R193-DEC-01); env unset → **inert**, byte-identical default profile (pinned by `tests/api/test_dev_router_mount_r172.py` and `tests/composition/test_r193_dev_bindings_composition.py`) | route absent (FastAPI generic 404, no `binding_id` echo) | `GET /v1/dev/bindings/{binding_id}/publish-modes` → `PublishModesResponse`; unknown / malformed / foreign-tenant id ⇒ ONE typed 404 `validation_error` with `details={binding_id}` only | Enumerate the publish modes a UI dropdown may bind to for a repository binding, once a composition root injects a binding registry (C2 store + C3 trust registry are the missing production pieces). |
