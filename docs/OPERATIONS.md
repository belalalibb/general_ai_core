...
| `VAULT_ADDR`, `VAULT_TOKEN`, `VAULT_MOUNT` | **Bound since R195-D (AD-2)**: the ONE shared `SecretManagerPort` becomes `VaultSecretManager` (KV v2 at `VAULT_MOUNT`, default `secret`); every `credential_ref` in the process — provider onboarding, gateway route tokens, dev repo bindings — resolves there. `VAULT_ADDR` without `VAULT_TOKEN` ⇒ startup `ValueError` (no silent fallback, 20 §5). Dev-binding credentials are seeded OUT-OF-BAND: write the token to `<VAULT_MOUNT>/<tenant_id>/<suffix>` with field `value`; the binding's `credential_ref` is `vault:<suffix>`. Rotation = seed a new suffix, publish a new binding, roll back the old one (D-03 evidence still owed). | absent ⇒ `InMemorySecretManager` (process-local; refs die with the process) |
...
| Browser proof of the Command Center EXPERIENCE (R185, D-R184-2; part of the `rest` gate slice) | same browser setup as the row above, then `pytest tests/ui/test_command_center_browser_r185.py -o addopts="" -q` — real Chromium against `/app/command/`: execution orbit == `GET /v1/executions` rows; rendered `evaluation_status` == the `GET /v1/admin/usage` row (R184); Core keyboard → system overview dialog (== `/v1/admin/system` + `/healthz`), Escape returns focus, 0 writes; node keyboard → record dialog; 390 px no horizontal overflow; `prefers-reduced-motion` → 0 running animations; no console errors; served routes only. It REWRITES `evidence/r185/{browser_proof_r185.json,command_center_r185_desktop.png,command_center_r185_mobile.png}` — run it in a scratch clone and copy the artefacts deliberately (F-R182I-06 posture) | yes (local) |
| Browser proof of the two recorded Command Center findings (R186, F-R185-L01/L02; part of the `rest` gate slice) | same browser setup, then `pytest tests/ui/test_command_center_fix_r186.py -o addopts="" -q` — static guard (wrapping rules on token-rendering elements; no stringified nullable `verification`) + real Chromium at 390 px: a REAL failed execution (`execution_policy.strategy=agent` → `invalid_proposal` → SSE `error` frame) and the RECORDED live Groq `invalid_credential` frame (`evidence/r185/live_preview/events.txt`, replayed through the events route) must give 0 px horizontal overflow; a `verification: null` converse response (substituted at the browser network layer — rendering proof) must render no `undefined`. It REWRITES `evidence/r186/{browser_proof_r186.json,error_frame_390.png,live_error_frame_390.png,null_verification_390.png}` — scratch clone, copy deliberately (F-R182I-06) | yes (local) |
