...
| C3 trust registry | **DONE R195 (AD-1, G-TRUST closed)**: the operator grant/revoke act is `AdminAction.GRANT_REMOTE_TRUST` / `REVOKE_REMOTE_TRUST` through the EXISTING `/v1/admin/changes` lifecycle (`AdminConfigService(remote_trust=dev.trust)`); publish writes the SAME `RemoteTrustRegistry` the per-tenant `GitToolset` consults; rollback restores the prior state auditably. Wiring itself was R193 (env-gated `JsonRemoteTrustStore`). | `core/admin/service.py`, `core/tools/remote_trust.py` |
...
| C7 dev seam | **DONE R193 + R195**: `runtime.py` passes `dev_bindings=dev.bindings` (R193); binding REGISTRATION is `AdminAction.REGISTER_REPO_BINDING` via the admin lifecycle (R195, **G-BIND closed**) — `/v1/dev` write routes remain deliberately absent (AD-1 ruling). Credential custody for bindings is the shared Vault `SecretManagerPort` when configured (R195-D, AD-2, **G-CRED closed**; seeding procedure in `docs/OPERATIONS.md §2`). | `apps/api/app.py`, `core/admin/service.py`, `apps/composition/runtime.py` |
| T1 templates surface | **DONE R196 (AD-3)**: `GET /v1/templates` + `GET /v1/templates/{ref}` (read-only, system templates, tenant-authenticated) over the ONE `TemplateRegistry` composed by `build_template_registry()`; `/v1/execute` `mode="template"` resolves `app_factory.plan` (R191-C closed). Baseline re-derived 44 → 46 (declared). Template durability / user-workspace ownership remain OPEN (P-R192-03; workspace = later direction). | `apps/api/app.py`, `apps/composition/runtime.py`, `core/contracts/agent_template.py` |
